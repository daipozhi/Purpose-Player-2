...
//#define  DDC_PI  (3.14159265358979323846264338327950288419)
  ...
  static long sr_fft_IsPowerOfTwo(long x);
  static long sr_fft_NumberOfBitsNeeded(long PowerOfTwo);
  static long sr_fft_ReverseBits(long index,long NumBits);
...
//
    ...
    static float sr_fft_Index_to_frequency (long NumSamples,long Index );

    static int   sr_fft_CheckPointer (float *p);
...
static    int     sr_fft_trans_all(VideoState *is);
...
// =============== 10 segment equalizer ==================

#define EQ_FFT_BUFFER_SIZE  4096
//#define EQ_FFT_BUFFER_SIZE  65536
...
static int eq_rate;
static int eq_ch;
static int eq_fft_start;
static int eq_fft_end;
static int eq_river_over;
static int eq_seri;

static int eq_sample_size;

static int eq_sample_over;
static int eq_sample_over2;

static short eq_buffer[9][10][EQ_FFT_BUFFER_SIZE];
static short eq_buffer2[EQ_FFT_BUFFER_SIZE*9*2];

static unsigned char eq_buffer_out[500000];
       int           eq_buffer_out_size;
       int           eq_buffer_out_indx;

#define   EQ_ADJ_NUM  40
...
static int eq_fft_trans_all(VideoState *is);
static int eq_fft_cx(int ch,int chn);

static int eq_smooth_proc(void);

static int eq_test(void);
...
//#define  DDC_PI  (3.14159265358979323846264338327950288419)
  ...
  static long eq_fft_IsPowerOfTwo(long x);
  static long eq_fft_NumberOfBitsNeeded(long PowerOfTwo);
  static long eq_fft_ReverseBits(long index,long NumBits);
...
//
    ...
    static float eq_fft_Index_to_frequency (long NumSamples,long Index );

    static int   eq_fft_CheckPointer (float *p);

// --------------------------------------------------------------------------

static    float  eq_fft_real_in1[EQ_FFT_BUFFER_SIZE];
static    float  eq_fft_real_ou1[EQ_FFT_BUFFER_SIZE];
static    float  eq_fft_imag_ou1[EQ_FFT_BUFFER_SIZE];
static    float  eq_fft_real_ou2[EQ_FFT_BUFFER_SIZE];
static    float  eq_fft_imag_ou2[EQ_FFT_BUFFER_SIZE];
...
static    float  get_eq_fft_real_in1(long addr);
static    float  get_eq_fft_real_ou1(long addr);
static    float  get_eq_fft_imag_ou1(long addr);
static    float  get_eq_fft_real_ou2(long addr);
static    float  get_eq_fft_imag_ou2(long addr);

static    int    set_eq_fft_real_in1(long addr,float val);
static    int    set_eq_fft_real_ou1(long addr,float val);
static    int    set_eq_fft_imag_ou1(long addr,float val);
static    int    set_eq_fft_real_ou2(long addr,float val);
static    int    set_eq_fft_imag_ou2(long addr,float val);
...
static int default_height =  750/*480*/;
...
static void fill_border(int xleft, int ytop, int width, int height, int x, int y, int w, int h)
{
    ...
    h2 = height - (y + h) -/*deb_ch_h*/18*2-9-18*5 ;
    ...
    fill_rectangle(
                   xleft, ytop,
                   w1, height -/*deb_ch_h*/18*2-9-18*5
                   );
    fill_rectangle(
                   xleft + width - w2, ytop,
                   w2, height -/*deb_ch_h*/18*2-9-18*5
                   );
    fill_rectangle(
                   xleft + w1, ytop,
                   width - w1 - w2, h1
    ...
    fill_rectangle(
                   xleft + w1, ytop + height - h2 -/*deb_ch_h*/18*2-9-18*5,
                   width - w1 - w2, h2
                   );
}
                                   ...
                                   int pic_width, int pic_height, AVRational pic_sar)
{
    ...
    // daipozhi modified
    y = (scr_height - height/**/ -18*2 -9 -18*5/**/) / 2;
...
static int video_open(VideoState *is)
{
    ...
    h =  750 /*screen_height ? screen_height : default_height*/ ;
...
static void update_sample_display(VideoState *is, short *samples, int samples_size)
{
    ...
    // -----------------------  daipozhi modified for equalizer  ---------------------------------------
    if (eq_init==0)
    {
        eq_init=1;
        eq_seri=0;
        //eq_buffer_out_indx=0;

	l=0;
	l=(EQ_FFT_BUFFER_SIZE*eq_ch)*(l/(EQ_FFT_BUFFER_SIZE*eq_ch));		// fft start and end
	m=l;
	eq_fft_start=l;
	eq_fft_end=m;
	...
	if (eq_fft_end<=is->sample_array_index)	// sample position of now
	{
	    l=eq_fft_end;		// fft start and end
	    m=(EQ_FFT_BUFFER_SIZE*eq_ch)*(is->sample_array_index/(EQ_FFT_BUFFER_SIZE*eq_ch));
	    eq_fft_start=l;
	    eq_fft_end=m;
	}
	else if (eq_sample_over==1)
	{
	    l=eq_fft_end; // ring buffer
	    m=(EQ_FFT_BUFFER_SIZE*eq_ch)*(is->sample_array_index/(EQ_FFT_BUFFER_SIZE*eq_ch));
	    eq_fft_start=l;
	    eq_fft_end=m;
	    eq_sample_over2=1;
	...
	{
	    eq_fft_start=eq_fft_end;
	}

	if (eq_fft_start==eq_fft_end)
	{
	  ret=1;
	}
	
	if ((eq_fft_start==eq_sample_size)&&(eq_fft_end==0))
	{
	  ret=1;
	}
	
#if DPZ_DEBUG1
	sprintf(m711_str1,"next,fft start=%d,fft end=%d,samp index=%d,",eq_fft_start,eq_fft_end,is->sample_array_index);
        deb_record(m711_str1);
#endif
    }

    // start fft
    if (ret==0) eq_fft_trans_all(is);
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    size=eq_buffer_out_size;
    indx=eq_buffer_out_indx;
    ...
    while (len > 0) {
        ...
        if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
            //memcpy(stream, (uint8_t *)is->audio_buf + is->audio_buf_index, len1);
            memcpy(stream, (uint8_t *)eq_buffer_out + indx, size1);
        ...
        // daipozhi modified
            ...
            memset(stream, 0, size1);
            if (!is->muted && is->audio_buf )
                  SDL_MixAudioFormat(stream, (uint8_t *)eq_buffer_out + indx, AUDIO_S16SYS, size1, is->audio_volume);
        ...
        eq_buffer_out_size =size;
        eq_buffer_out_indx =indx;
    ...
    //daipozhi modified
    is->audio_write_buf_size = size - indx;
...
static int audio_open(void *opaque, AVChannelLayout *wanted_channel_layout, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    eq_sample_size=((SAMPLE_ARRAY_SIZE/(spec.channels*EQ_FFT_BUFFER_SIZE)))*spec.channels*EQ_FFT_BUFFER_SIZE;
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
	...
	//daipozhi modified
        ...
        remaining_time = REFRESH_RATE;
                ...
                // daipozhi modified for sound river
		if ((sr_show_start==1)&&(sr_time_set==0)&&(deb_thr_a))
		{
...
static void event_loop(VideoState *cur_stream)
{
    ...
    */
    // test end 
	    ...
	    if (g_detect_ptr1==2)
	    {
	      if ((g_detect_ptr2<0)||(g_detect_ptr2>=G_SCROLLBAR_NUM)||(g_detect_ptr2>=g_scrollbar_ptr)) break;
	        
	      g_set_scrollbar_value_now(g_detect_ptr2 , g_get_scrollbar_value_max(g_detect_ptr2)*(float)g_detect_ptr3/(float)g_detect_ptr4 );
	        
	      if (g_detect_ptr2==0) // in scroll bar (seek bar)
	      {
	        if ((screen_w<640)||(screen_w>7680)) break;
	        if ((screen_h<480)||(screen_h>4320)) break;
		...
		if (deb_stream_open==1) break;
		  ...
		  if (deb_eo_stream==1) break;
                  ...
                  if (seek_by_bytes || cur_stream->ic->duration <= 0) {
                    uint64_t size =  avio_size(cur_stream->ic->pb);
                    stream_seek(cur_stream, size*x/(g_detect_ptr4), 0, 1);
                  } else {
                    int64_t ts;
                    int ns, hh, mm, ss;
                    int tns, thh, tmm, tss;
	  ...
	  else
	  {
        ...
        case SDL_WINDOWEVENT:
		    ...
		    g_move_dirview(0,0,0,cur_stream->width,cur_stream->height-18*2-9-18*5);
		    

                    // for testing of mini GUI lib
                    /*
       		    g_move_dirview(0,0,18*7+1,cur_stream->width,cur_stream->height-18*9-9-18*5);
       		    */
       		    // test end
		    ...
		    g_move_scrollbar( 2,(cur_stream->width-50*12)/2+ 50,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 3,(cur_stream->width-50*12)/2+100,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 4,(cur_stream->width-50*12)/2+150,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 5,(cur_stream->width-50*12)/2+200,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 6,(cur_stream->width-50*12)/2+250,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 7,(cur_stream->width-50*12)/2+300,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 8,(cur_stream->width-50*12)/2+350,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar( 9,(cur_stream->width-50*12)/2+400,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar(10,(cur_stream->width-50*12)/2+450,cur_stream->height-18*2-18*4,16,18*4);
		    g_move_scrollbar(11,(cur_stream->width-50*12)/2+500,cur_stream->height-18*2-18*4,16,18*4);
		    ...
		    g_move_label( 1,(cur_stream->width-50*12)/2+ 50-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 2,(cur_stream->width-50*12)/2+100-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 3,(cur_stream->width-50*12)/2+150-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 4,(cur_stream->width-50*12)/2+200-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 5,(cur_stream->width-50*12)/2+250-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 6,(cur_stream->width-50*12)/2+300-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 7,(cur_stream->width-50*12)/2+350-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 8,(cur_stream->width-50*12)/2+400-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label( 9,(cur_stream->width-50*12)/2+450-10,cur_stream->height-18*2-18*4-16,35,16);
		    g_move_label(10,(cur_stream->width-50*12)/2+500-10,cur_stream->height-18*2-18*4-16,35,16);



		    g_move_label(11,(cur_stream->width-50*12)/2-7,cur_stream->height-18*2-18*4-1,35,16);
		    g_move_label(12,(cur_stream->width-50*12)/2-7,cur_stream->height-18*2-18*2-9,35,16);
		    g_move_label(13,(cur_stream->width-50*12)/2-7,cur_stream->height-18*2-18*1+3,35,16);



		    g_move_label(14,(cur_stream->width-50*12)/2+500+25,cur_stream->height-18*2-18*4-1,35,16);
		    g_move_label(15,(cur_stream->width-50*12)/2+500+25,cur_stream->height-18*2-18*2-9,35,16);
		    g_move_label(16,(cur_stream->width-50*12)/2+500+25,cur_stream->height-18*2-18*1+3,35,16);
...
int main(void/*int argc, char **argv*/)
{
    ...
    g_create_dirview(0,0,is->width,is->height-18*2-9-18*5);
    
    
    // for testing of mini GUI lib
    /*
    g_create_dirview(0,18*7+1,is->width,is->height-18*9-9-18*5);
    */
    // test end
    ...
    g_create_scrollbar((is->width-50*12)/2+ 50,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+100,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+150,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+200,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+250,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+300,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+350,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+400,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+450,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    g_create_scrollbar((is->width-50*12)/2+500,is->height-18*2-18*4,16,18*4,0,224,224,224,1);
    ...
    strcpy(str1,"20hz");
    g_create_label((is->width-50*12)/2+50-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"50hz");
    g_create_label((is->width-50*12)/2+100-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"100hz");
    g_create_label((is->width-50*12)/2+150-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"200hz");
    g_create_label((is->width-50*12)/2+200-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"500hz");
    g_create_label((is->width-50*12)/2+250-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"1Khz");
    g_create_label((is->width-50*12)/2+300-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"2Khz");
    g_create_label((is->width-50*12)/2+350-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"5Khz");
    g_create_label((is->width-50*12)/2+400-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"10Khz");
    g_create_label((is->width-50*12)/2+450-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"20Khz");
    g_create_label((is->width-50*12)/2+500-10,is->height-18*2-18*4-16,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);



    strcpy(str1,"+20db");
    g_create_label((is->width-50*12)/2-7,is->height-18*2-18*4-1,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"  0db");
    g_create_label((is->width-50*12)/2-7,is->height-18*2-18*2-9,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"-20db");
    g_create_label((is->width-50*12)/2-7,is->height-18*2-18*1+3,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);



    strcpy(str1,"+20db");
    g_create_label((is->width-50*12)/2+500+25,is->height-18*2-18*4-1,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"  0db");
    g_create_label((is->width-50*12)/2+500+25,is->height-18*2-18*2-9,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);

    strcpy(str1,"-20db");
    g_create_label((is->width-50*12)/2+500+25,is->height-18*2-18*1+3,35,16,0,240,240,240,str1,6);
    g_set_label_text_color(1,1,0,0,0);
    ...
    */
    ...
    g_set_scrollbar_value_max( 2,200);
    g_set_scrollbar_value_now( 2,100);

    g_set_scrollbar_value_max( 3,200);
    g_set_scrollbar_value_now( 3,100);

    g_set_scrollbar_value_max( 4,200);
    g_set_scrollbar_value_now( 4,100);

    g_set_scrollbar_value_max( 5,200);
    g_set_scrollbar_value_now( 5,100);

    g_set_scrollbar_value_max( 6,200);
    g_set_scrollbar_value_now( 6,100);

    g_set_scrollbar_value_max( 7,200);
    g_set_scrollbar_value_now( 7,100);

    g_set_scrollbar_value_max( 8,200);
    g_set_scrollbar_value_now( 8,100);

    g_set_scrollbar_value_max( 9,200);
    g_set_scrollbar_value_now( 9,100);

    g_set_scrollbar_value_max(10,200);
    g_set_scrollbar_value_now(10,100);

    g_set_scrollbar_value_max(11,200);
    g_set_scrollbar_value_now(11,100);
...
static int bt_old_node(int ptr)
{
   ...
   {
     printf("In btree1,error at bt_old_node()\n");
     return(-1);
   }
   else
   {
  ...
  while(1)
  {
        ...
        for (j=0;j<sr_ch;j=j+sr_ch)
	{
          for (m=0;m<SR_FFT_BUFFER_SIZE;m++)
	  {
		...
		//n=n/3;   // reduce volumn , prevent 16bit int overflow , when you increase volumn by equalizer and sum these audio , 16bit int will overflow
		             ...
		             float *ImagOut )
{
   ...
   if ( !sr_fft_IsPowerOfTwo(NumSamples) ) return(1);

   if (sr_fft_CheckPointer(RealIn )) return(1);
   if (sr_fft_CheckPointer(RealOut)) return(1);
   if (sr_fft_CheckPointer(ImagOut)) return(1);

   NumBits=sr_fft_NumberOfBitsNeeded(NumSamples);
   ...
   */
   ...
   {
      j = sr_fft_ReverseBits(i,NumBits );
...
so   d2=(h-14*2-7)/151
*/  
  d2=((float)screen_h-18*2-9-18*5-7-14-10-10)/151;
...
static int  sr_river_show(VideoState *cur_stream)
{
  ...
  // display comment
  deb_echo_str4screenstring(10,screen_h -18*2 -9-18*5-14-10,"Every Column Has 256 Samples(44.1KHz)(5.80ms)",45+1,2,0);
...
static int  sr_river_f_cons_test(VideoState *cur_stream,int ptr)
{
  ...
  fill_rectangle(0 ,0 , cur_stream->width , cur_stream->height -/*deb_ch_h*/ 18*2 -9 -18*5); 
  ...
  // display comment
  deb_echo_str4screenstring(10,screen_h -18*2 -9 -18*5-14-10,"Every Column Has 256 Samples(44.1KHz)(5.80ms)",45+1,0,1);
...
int clr_rect_black(void)
{
	...
	j=stream_open_is->width * (stream_open_is->height-18*2-9-18*5);
	for (i=0;i<k;i++)
	{
	  if (i<j)
	  {
        ...
        g_draw_line(0,stream_open_is->height-18*2-9-18*5,stream_open_is->width,stream_open_is->height-18*2-9-18*5,0,128,128,128);
    ...
    else if (hori==1)
    {
	  ...
	  if      ((n==0)||(n==m-1)) { j1=128;j2=128;j3=128; }
          ...
          if (g_scrollbar_enable[i]==1)
          {
            ...
            {
                  SDL_SetRenderDrawColor(renderer, j1, j2, j3, i4);
                  fill_rectangle(q+o,k+n,/* q+o,*/ 1, 1);
            }
            else
            {
	  ...
	  if      ((n==0)||(n==4  )) { j1=128;j2=128;j3=128; }
          ...
          if (g_scrollbar_enable[i]==1)
          {
            ...
            {
                  SDL_SetRenderDrawColor(renderer, j1, j2, j3, i4);
                  fill_rectangle(t+o,r+n,/* t+o,*/ 1, 1);
            }
            else
            {
...
static int eq_fft_trans_all(VideoState *is)
{
  ...
  while(1)
  {
    if (((eq_sample_over2==0)&&(l<eq_fft_end))||
	((eq_sample_over2==1)&&(l<eq_sample_size)))
    {
        if (ini==0)
        {
          ini=1;
	  eq_buffer_out_size=0;
        }
        
        // trans for left and right channel to many
        for (j=0;j<eq_ch;j++)
	{
          for (m=0;m<EQ_FFT_BUFFER_SIZE;m++)
	  {
		p=l+m*eq_ch+j;

		if ((p<0)||(p>=eq_sample_size)) continue;

		n=is->sample_array[p];
		
		n=n/5;   // reduce volumn , prevent 16bit int overflow , when you increase volumn by equalizer and sum these audio , 16bit int will overflow

	        set_eq_fft_real_in1(m,(float)n);
	  }

	  // transform to freq
	  i=eq_fft_float(EQ_FFT_BUFFER_SIZE,eq_fft_real_in1,eq_fft_real_ou1,eq_fft_imag_ou1);
	  if (i!=0) return(1);

	  // store to buffer for multiple get
	  for (m=0;m<EQ_FFT_BUFFER_SIZE;m++)
	  {
		d1=get_eq_fft_real_ou1(m);
        	set_eq_fft_real_ou2(m,d1);

		d1=get_eq_fft_imag_ou1(m);
        	set_eq_fft_imag_ou2(m,d1);
	  }
	...
	for (i=0;i<10;i++)  // 10 segment equalizer
	{
	  k=g_get_scrollbar_value_now(i+2);
	  ...
	  else
	  {
	    ...
	    for (q=0;q<k;q++) f1=f1*0.89;
	  }
	  
	  for (j=0;j<eq_ch;j++)
	  {
	    for (q=0;q<EQ_FFT_BUFFER_SIZE;q++)
	    {
	      eq_buffer[j][i][q]=(float)eq_buffer[j][i][q]*f1;
	    }
	  }
	}
	...
	if (eq_seri==0)  //1st block
	{
	  ...
	  for (j=0;j<eq_ch;j++) // 
	  {
	    for (q=0;q<EQ_FFT_BUFFER_SIZE;q++)
	    {
	      ...
	      eq_buffer2[q*eq_ch+j]=r;
	    }
	  }
	}
	else if (eq_seri==1) //2nd block
	{
	  eq_seri++;
	  ...
	  for (j=0;j<eq_ch;j++) // 
	  {
	    for (q=0;q<EQ_FFT_BUFFER_SIZE;q++)
	    {
	      ...
	      eq_buffer2[EQ_FFT_BUFFER_SIZE*eq_ch+q*eq_ch+j]=r;
	    }
	  }
	  
	  v=1;
	}
	else //3rd block or more    
	{
	  for (j=0;j<eq_ch;j++) //  // move forward
	  {
	    for (q=0;q<EQ_FFT_BUFFER_SIZE;q++)
	    {
	      eq_buffer2[q*eq_ch+j]=eq_buffer2[EQ_FFT_BUFFER_SIZE*eq_ch+q*eq_ch+j];
	    }
	  }
	  	  
	  for (j=0;j<eq_ch;j++) //   // new block
	  {
	    for (q=0;q<EQ_FFT_BUFFER_SIZE;q++)
	    {
	      r=0;
	...
	if (v==1)
	{
	  for (j=0;j<eq_ch;j++) // smooth proccess
	  {
	    for (t=0;t<EQ_ADJ_NUM/2;t++) eq_m[t]=eq_buffer2[(EQ_FFT_BUFFER_SIZE-EQ_ADJ_NUM/2+t)*eq_ch+j];
	    
	    for (t=0;t<EQ_ADJ_NUM/2;t++) eq_m[EQ_ADJ_NUM/2+t]=eq_buffer2[(EQ_FFT_BUFFER_SIZE+t)*eq_ch+j];

	    eq_smooth_proc();
	  
	    for (t=0;t<EQ_ADJ_NUM/2;t++) eq_buffer2[(EQ_FFT_BUFFER_SIZE-EQ_ADJ_NUM/2+t)*eq_ch+j]=eq_m[t];
	    
	    for (t=0;t<EQ_ADJ_NUM/2;t++) eq_buffer2[(EQ_FFT_BUFFER_SIZE+t)*eq_ch+j]=eq_m[EQ_ADJ_NUM/2+t];
	  }
	  ...
	  out=(int16_t *)eq_buffer_out;
	  
	  for (j=0;j<eq_ch;j++) //  // move forward
	  {
	    for (q=0;q<EQ_FFT_BUFFER_SIZE;q++)
	    {
	      out[eq_buffer_out_size/2+q*eq_ch+j]=/* 3* */eq_buffer2[q*eq_ch+j]; // 3*... resume volumn
	    }
	  }
	  
	  eq_buffer_out_size=eq_buffer_out_size+EQ_FFT_BUFFER_SIZE*eq_ch*2;
	}
	
	
	
	l=l+EQ_FFT_BUFFER_SIZE*eq_ch;

	if (eq_sample_over2==1)  //ring buffer
	{
	  if (l>=eq_sample_size)
	  {
	    l=l-eq_sample_size;
...
static int eq_fft_cx(int ch,int chn)
{
  ...
  if ((ch<0)||(ch>=eq_ch)) return(0);
  if ((chn<0)||(chn>=10)) return(0);
  
  if (EQ_FFT_BUFFER_SIZE==4096)
  {
    if      (chn==0) { /*chn1=0;    chn2=2;   */ chn1=0;    chn2=3;    } //20hz
    else if (chn==1) { /*chn1=3;    chn2=5;   */ chn1=4;    chn2=7;    } //50hz
    else if (chn==2) { /*chn1=6;    chn2=10;  */ chn1=8;    chn2=15;   } //100hz
    else if (chn==3) { /*chn1=11;   chn2=20;  */ chn1=16;   chn2=35;   } //200hz
    else if (chn==4) { /*chn1=21;   chn2=50;  */ chn1=36;   chn2=75;   } //500hz
    else if (chn==5) { /*chn1=51;   chn2=100; */ chn1=76;   chn2=150;  } // 1Khz
    else if (chn==6) { /*chn1=101;  chn2=200; */ chn1=151;  chn2=350;  } // 2Khz
    else if (chn==7) { /*chn1=201;  chn2=500; */ chn1=351;  chn2=750;  } // 5Khz
    else if (chn==8) { /*chn1=501;  chn2=1000;*/ chn1=751;  chn2=1523; } //10khz
    else             { /*chn1=1001; chn2=2047;*/ chn1=1524; chn2=2047; } //22Khz
  }
  else if (EQ_FFT_BUFFER_SIZE==65536)
  {
    if      (chn==0) { /*chn1=0;     chn2=29;   */ chn1=0;      chn2=51;    } //20hz
    else if (chn==1) { /*chn1=30;    chn2=74;   */ chn1=52;     chn2=111;   } //50hz
    else if (chn==2) { /*chn1=75;    chn2=148;  */ chn1=112;    chn2=222;   } //100hz
    else if (chn==3) { /*chn1=149;   chn2=297;  */ chn1=223;    chn2=520;   } //200hz
    else if (chn==4) { /*chn1=298;   chn2=743;  */ chn1=521;    chn2=1114;  } //500hz
    else if (chn==5) { /*chn1=744;   chn2=1486; */ chn1=1115;   chn2=2229;  } // 1Khz
    else if (chn==6) { /*chn1=1487;  chn2=2972; */ chn1=2230;   chn2=5201;  } // 2Khz
    else if (chn==7) { /*chn1=2973;  chn2=7430; */ chn1=5202;   chn2=11145; } // 5Khz
    else if (chn==8) { /*chn1=7431;  chn2=14860;*/ chn1=11146;  chn2=23813; } //10khz
    else             { /*chn1=14861; chn2=32767;*/ chn1=23814;  chn2=32767; } //22Khz
  }
  
  // get 1 of 10 chn data each times
  for (m=0;m<EQ_FFT_BUFFER_SIZE;m++)
  {
      if ((m-0>=chn1)&&(m-0<=chn2)              ||
          (EQ_FFT_BUFFER_SIZE-1-m>=chn1)&&(EQ_FFT_BUFFER_SIZE-1-m<=chn2)  )
      {
	d1=get_eq_fft_real_ou2(m);
	set_eq_fft_real_in1(m,d1);

	d1=get_eq_fft_imag_ou2(m);
	set_eq_fft_imag_ou1(m,d1);
      }
      else
      {
	set_eq_fft_real_in1(m,0);
        set_eq_fft_imag_ou1(m,0);
      }
  }

  // transform to pcm wave data
  i=eq_ifft_float(EQ_FFT_BUFFER_SIZE,eq_fft_real_in1,eq_fft_real_ou1,eq_fft_imag_ou1);
  if (i!=0) return(1);

  for (m=0;m<EQ_FFT_BUFFER_SIZE;m++)
  {
      lj=get_eq_fft_real_ou1(m);
      
      eq_buffer[ch][chn][m]=lj;
  }
...
static int eq_test(void)
{
  ...
  printf("start...\n");
  printf("m1-19=%6d,%6d,%6d,%6d,%6d,%6d,%6d,%6d,%6d,%6d,\n",eq_m[0],eq_m[1],eq_m[2],eq_m[3],eq_m[4],eq_m[5],eq_m[6],eq_m[7],eq_m[8],eq_m[9]);
  printf("m1-19=%6d,%6d,%6d,%6d,%6d,%6d,%6d,%6d,%6d,%6d,\n",eq_m[10],eq_m[11],eq_m[12],eq_m[13],eq_m[14],eq_m[15],eq_m[16],eq_m[17],eq_m[18],eq_m[19]);
...
static int eq_smooth_proc(void)
{
  ...
  if ((eq_m[0]==0)&&(eq_m[EQ_ADJ_NUM-1]==0))
  {
    for (i=1;i<EQ_ADJ_NUM-1;i++) eq_m[i]=0;
  }
  else
  {
    n_ptr=0;
  
    if (eq_m[0]<0) i1=0-eq_m[0];
    else           i1=eq_m[0];

    if (eq_m[EQ_ADJ_NUM-1]<0) i2=0-eq_m[EQ_ADJ_NUM-1];
    else            i2=eq_m[EQ_ADJ_NUM-1];

    if (i2>i1)
    {
      n_code[n_ptr]=1;  // mirror convert

      for (i=0;i<EQ_ADJ_NUM;i++)  n[n_ptr][i]=eq_m[EQ_ADJ_NUM-1-i];
    }
    else
    {
      n_code[n_ptr]=0;  // copy
      
      for (i=0;i<EQ_ADJ_NUM;i++)  n[n_ptr][i]=eq_m[i];
    }
    ...
    if (n[0][0]<0)
    {
      n_code[n_ptr]=2;  // up side down convert
      
      for (i=0;i<EQ_ADJ_NUM;i++)  n[n_ptr][i]=0-n[0][i];
    }
    else
    {
      n_code[n_ptr]=0;  // copy
      
      for (i=0;i<EQ_ADJ_NUM;i++)  n[n_ptr][i]=n[0][i];
    }
      
    n_ptr++;


    j=EQ_STEP;      //smooth proc
    f1=1000000.0;
    i3=1;
      
    for (i=1;i<j;i++)
    {
      f2=(float)n[1][EQ_ADJ_NUM-1]-(float)n[1][0]*cos(  ((float)i/(float)j)  *  3.14  );
      if (f2<0) f2=0-f2;
      if (f1>f2)
      {
        f1=f2;
    ...
    for (i=1;i<=EQ_ADJ_NUM-2;i++)
    {
      n[1][i]=(float)n[1][0]*cos(   ((float)i/(float)(EQ_ADJ_NUM-1))  *  ((float)i3/(float)j)  *  3.14  );
    }
    
    
    n_ptr--;
      
    if (n_code[n_ptr]==2)  // undo up side down convert
    {
        for (i=0;i<EQ_ADJ_NUM;i++)  n[n_ptr-1][i]=0-n[n_ptr][i];
    }
    else     //copy
    {
        for (i=0;i<EQ_ADJ_NUM;i++)  n[n_ptr-1][i]=n[n_ptr][i];
    }
      
    n_ptr--;
      
    if (n_code[n_ptr]==1) // undo mirror convert
    {
        for (i=0;i<EQ_ADJ_NUM;i++)  eq_m[i]=n[n_ptr][EQ_ADJ_NUM-1-i];
    }
    else     //copy
    {
        for (i=0;i<EQ_ADJ_NUM;i++)  eq_m[i]=n[n_ptr][i];
    }
...
static int eq_fft_float( long    NumSamples,
			     float *RealIn,
		             float *RealOut,
		             float *ImagOut )
{
   ...
   if ( !eq_fft_IsPowerOfTwo(NumSamples) ) return(1);

   if (eq_fft_CheckPointer(RealIn )) return(1);
   if (eq_fft_CheckPointer(RealOut)) return(1);
   if (eq_fft_CheckPointer(ImagOut)) return(1);

   NumBits=eq_fft_NumberOfBitsNeeded(NumSamples);
   ...
   */
   ...
   for ( i=0; i < NumSamples; i++ )
   {
      j = eq_fft_ReverseBits(i,NumBits );
...
static int eq_ifft_float(long     NumSamples,
			     float  *RealIn,
		             float  *RealOut,
		             float  *ImagOut )
{
...
static float  get_eq_fft_real_in1(long addr)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	return(eq_fft_real_in1[addr]);
}
static float  get_eq_fft_real_ou1(long addr)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	return(eq_fft_real_ou1[addr]);
}
static float  get_eq_fft_imag_ou1(long addr)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	return(eq_fft_imag_ou1[addr]);
}
static float  get_eq_fft_real_ou2(long addr)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	return(eq_fft_real_ou2[addr]);
}
static float  get_eq_fft_imag_ou2(long addr)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	return(eq_fft_imag_ou2[addr]);
}

static int  set_eq_fft_real_in1(long addr,float val)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	eq_fft_real_in1[addr]=val;
	return(0);
}
static int  set_eq_fft_real_ou1(long addr,float val)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	eq_fft_real_ou1[addr]=val;
	return(0);
}
static int  set_eq_fft_imag_ou1(long addr,float val)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	eq_fft_imag_ou1[addr]=val;
	return(0);
}
static int  set_eq_fft_real_ou2(long addr,float val)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	eq_fft_real_ou2[addr]=val;
	return(0);
}
static int  set_eq_fft_imag_ou2(long addr,float val)
{
	if ((addr<0)||(addr>=EQ_FFT_BUFFER_SIZE)) return(0);
	eq_fft_imag_ou2[addr]=val;
	return(0);
}
