...
static int  u_n1, u_n2, u_n3, u_n4;
static int  u_nb = 1;
static int  u_ptr = 0;
static int  u_cptr = 0;
               ...
               else if (deb_sr_rate==48000)
               {
                 ...
                 for (n=0;n<deb_sr_ch;n++)
                 {
                   ...
                   while (1)
                   {
                     ...
                     if (160*rept-start+m<j/deb_sr_ch)
                     {
                       deb_resam_buff2[i*deb_sr_ch+n]=deb_resam_buff1[(160*rept-start+m)*deb_sr_ch+n];
                       if (size<i) size=i;
                     }
                     else
                     {
               ...
               else if (deb_sr_rate==32000)
               {
                 ...
                 for (n=0;n<deb_sr_ch;n++)
                 {
                   ...
                   while (1)
                   {
                     ...
                     if (320*rept-start+m<j/deb_sr_ch)
                     {
                       deb_resam_buff2[i*deb_sr_ch+n]=deb_resam_buff1[(320*rept-start+m)*deb_sr_ch+n];
                       if (size<i) size=i;
                     }
                     else
                     {
...
// daipozhi modified for sound river
#if DPZ_DEBUG2
	len2=0;  //to playback high freq,low freq,middle freq audio,to sure fft is ok,need click '[river on]',set DPZ_DEBUG2 to 1,press key 'm' to switch chanel
	len3=0;
	stream2=stream;
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
	...
	// daipozhi modified
        ...
        if (remaining_time > 0.0)
            av_usleep((int64_t)(remaining_time * 1000000.0 /**/ * 0.33/**/ )); // notice '*0.33'
            
            
            
            
        //daipozhi modified , notice '*0.33'
        remaining_time = (double)REFRESH_RATE /**/* 0.33/**/ ;
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:
            ...
            av_log(NULL, AV_LOG_INFO, "%s", "key down\n");
...
static int deb_load_font(void)
{
  ...
  u_str = (unsigned char*)u_b1p;  read(deb_fh, u_str,   128 * 14 * 14);
  u_str = (unsigned char*)u_b2p;  read(deb_fh, u_str,  1920 * 14 * 14);
  u_str = (unsigned char*)u_b3p;  read(deb_fh, u_str,  2048 * 14 * 14);
  u_str = (unsigned char*)u_b4p;  read(deb_fh, u_str, 49152 * 14 * 14);
  u_str = (unsigned char*)u_b5p;  read(deb_fh, u_str,  2048 * 14 * 14);
  u_str = (unsigned char*)u_b6p;  read(deb_fh, u_str,  8192 * 14 * 14);

  u_str = (unsigned char*)u_b1s;  read(deb_fh, u_str,   128 * 2);
  u_str = (unsigned char*)u_b2s;  read(deb_fh, u_str,  1920 * 2);
  u_str = (unsigned char*)u_b3s;  read(deb_fh, u_str,  2048 * 2);
  u_str = (unsigned char*)u_b4s;  read(deb_fh, u_str, 49152 * 2);
  u_str = (unsigned char*)u_b5s;  read(deb_fh, u_str,  2048 * 2);
...
static char deb_getfirstchar(char *buffer,int buffer_size)
{
  ...
  if (deb_str_has_null(buffer,buffer_size)!=1) return(' ');
  
  k=(int)strlen(buffer);
  if (k<=0) return(' ');
  //if (k>buffer_size) k=buffer_size;
