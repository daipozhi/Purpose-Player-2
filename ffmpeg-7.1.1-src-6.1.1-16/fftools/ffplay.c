...
#define G_SCROLLBAR_NUM   20
#define G_DIRVIEW_NUM      5
#define G_LABEL_NUM       30
...
int strltrim(char *p_str,int p_str_size);

int g_mouse_cntr;
int g_def_proc(SDL_Event event,int vid);
int g_def_proc2(SDL_Event event,int vid);
int g_def_proc_nopaint(SDL_Event event,int vid);
int g_def_proc_paint(int vid);


#define G_MENU_NUM 10

char g_menu_item_father[G_MENU_NUM][200][10];
char g_menu_item_name[G_MENU_NUM][200][300];
char g_menu_item_self[G_MENU_NUM][200][10];
char g_menu_item_separate[G_MENU_NUM][200];
char g_menu_item_haschild[G_MENU_NUM][200];
char g_menu_item_load[G_MENU_NUM];
int  g_menu_item_ptr[G_MENU_NUM];

int  g_menu_load(int ptr);

#define G_MENU_PAINT_NUM 8

int  g_menu_paint_x[G_MENU_PAINT_NUM];
int  g_menu_paint_y[G_MENU_PAINT_NUM];
int  g_menu_paint_n1[G_MENU_PAINT_NUM];
int  g_menu_paint_n2[G_MENU_PAINT_NUM];
int  g_menu_paint_w[G_MENU_PAINT_NUM];
int  g_menu_paint_h[G_MENU_PAINT_NUM];
int  g_menu_paint_menu[G_MENU_PAINT_NUM];
int  g_menu_paint_code[G_MENU_PAINT_NUM];

int  g_menu_paint_ptr;

int g_menu_paint_prepare(int x,int y,int menuptr,char *father);
int g_paint_menu(void);

// --------------------------- end ----------------------------------

   VideoState       *stream_open_is;
 ...
 int deb_str_has_null(char *in_str,int in_str_size);
...
int deb_ini_is(VideoState *is);

void deb_fill_rectangle(int x, int y, int w, int h);

  int  deb_clr_rect_white(void);
  int  deb_clr_rect_black(void);

 	char   deb_lower(char c1);
 	int    deb_lower_string(char *p_instr,int p_instr_size);
 	char   deb_upper(char c1);
 	int    deb_upper_string(char *p_instr,int p_instr_size);

 	int    deb_string2int(char *string,int p1,int p2);

  int    deb_utf8_to_gb18030(char *inbuffer,char *outbuffer,int outbufferlen);
  int    deb_gb18030_to_utf8(char *inbuffer,char *outbuffer,int outbufferlen);

  int string_cpy(char *p_outstr,int p_outstr_size,char *p_instr,int p_instr_size);

// ========================= file tree ===============================
#define FN_SIZE       300
#define MAX_FILE_NUM  100000

  char   ftree_filenamebuff[MAX_FILE_NUM][FN_SIZE];
  char   ftree_filenamebuff_ext[MAX_FILE_NUM][6];
  char   ftree_filenamebuff_size[MAX_FILE_NUM][7];
  char   ftree_filenamebuff_date[MAX_FILE_NUM][21];
  int    ftree_filenamebuff_len[MAX_FILE_NUM];
  int    ftree_filenamebuff_len2[MAX_FILE_NUM];
  char   ftree_filenamebuff_type[MAX_FILE_NUM];
  char   ftree_filenamebuff_icon[MAX_FILE_NUM];
  int    ftree_filenamebuff_space[MAX_FILE_NUM];
  char   ftree_filenamebuff_subline[MAX_FILE_NUM];
  char   ftree_filenamebuff_select[MAX_FILE_NUM];
  int    ftree_filenamebuff_ptr;
  int    ftree_filenamebuff_n;
  int    ftree_filenamebuff_n2;
  int    ftree_filenameplay;
  char   ftree_currentpath[FN_SIZE];


  char   ftree_filenamebuff2[MAX_FILE_NUM][FN_SIZE];
  char   ftree_filenamebuff2_ext[MAX_FILE_NUM][6];
  char   ftree_filenamebuff2_size[MAX_FILE_NUM][7];
  char   ftree_filenamebuff2_date[MAX_FILE_NUM][21];
  int    ftree_filenamebuff2_len[MAX_FILE_NUM];
  int    ftree_filenamebuff2_len2[MAX_FILE_NUM];
  char   ftree_filenamebuff2_type[MAX_FILE_NUM];
  char   ftree_filenamebuff2_icon[MAX_FILE_NUM];
  int    ftree_filenamebuff2_space[MAX_FILE_NUM];
  char   ftree_filenamebuff2_subline[MAX_FILE_NUM];
  char   ftree_filenamebuff2_select[MAX_FILE_NUM];
  int    ftree_filenamebuff2_ptr;


// daipozhi modified 
 	int    ftree_read_dir(void);
 	int    ftree_disp_dir(VideoState *is);
 	int    ftree_conv_dir(void);

 	int    ftree_filename_ext(char *path,int path_size,char *fext,int fext_size);
 	int    ftree_filename_dir(char *path,char *name);

  char   ftree_dir_buffer[FN_SIZE];

// daipozhi modified 
 	int  ftree_read_dir_ini(void);

 	int  ftree_dir_opened(int ptr );
 	int  ftree_get_space(int ptr);
 	int  ftree_get_space2(int ptr);
 	int  ftree_dir_add_after(int ptr);
 	int  ftree_dir_remove_after(int ptr);
 	int  ftree_get_path(int ptr);
 	int  ftree_get_path1(char *buffer1,char *buffer2);
 	int  ftree_get_path2(char *buffer1,char *buffer2);
 	int  ftree_cmp_dir(char *buffer1,char *buffer2);

  struct stat   deb_m_info;
  struct tm*    deb_m_info_tm;
  int           deb_m_info_len;
  char          deb_m_info_type;

 int ftree_size_format(int pn,char *buffer);

    long long int ftree_time1;      // for mouse double click
    long long int ftree_time2;
    long long int ftree_time3;
    long long int ftree_time4;
    long long int ftree_time5;
    int           ftree_time_set1;
    int           ftree_time_set2;
...
int  pl_play_selected;
int  pl_play_selected_repeatedly;
char pl_play_selected_list[3000][FN_SIZE];
char pl_play_selected_list_type[3000];
int  pl_play_selected_list_ptr;
int  pl_play_selected_list_play;
...
#define BTREE1_SIZE   100000
#define BTREE1_LSIZE  100000

    char  bt_node_mark[BTREE1_SIZE];
    char  bt_node_val[BTREE1_SIZE][FN_SIZE];
    char  bt_node_val2[BTREE1_SIZE];
    char  bt_node_val3[BTREE1_SIZE][6];
    char  bt_node_val4[BTREE1_SIZE][7];
    char  bt_node_val5[BTREE1_SIZE][21];
    char  bt_node_val6[BTREE1_SIZE];

//file name compare
    char  bt_fnc_str[BTREE1_SIZE][FN_SIZE];
    char  bt_fnc_msk[BTREE1_SIZE][FN_SIZE];
    int   bt_fnc_istr[BTREE1_SIZE][FNC_NUM];
    int   bt_fnc_ptr1[BTREE1_SIZE];
    int   bt_fnc_ptr2[BTREE1_SIZE];

    int   bt_stack[BTREE1_SIZE];
    int   bt_stack_ptr;
    ...
    int   bt_list_stack[BTREE1_LSIZE];
    char  bt_list_stack_type[BTREE1_LSIZE];
    int   bt_list_ptr;

     char  bt_out_buff[BTREE1_SIZE][FN_SIZE];
     char  bt_out_buff2[BTREE1_SIZE];
     char  bt_out_buff3[BTREE1_SIZE][6];
     char  bt_out_buff4[BTREE1_SIZE][7];
     char  bt_out_buff5[BTREE1_SIZE][21];
     char  bt_out_buff6[BTREE1_SIZE];
     int   bt_out_ptr;

    int   bt_err;

    int   bt_init_tree(void);
    int   bt_new_node(void);
    int   bt_old_node(int ptr);
    int   bt_clear_node(int ptr);
    int   bt_search_node(char *pstr,char ptype);
    int   bt_insert_node(char *pstr,char ptype);
    int   bt_delete_node(char *pstr,char ptype);
    int   bt_smallest(void);
    int   string_comp(char *ps1,char *ps2);
    int   bt_after_list(void);
    int   bt_out_list(char *pstr,char ptype,int ptr);
//    int   bt_dsp_list(void);
//    int   bt_save_list(char *fn);
...
//#define FNC_NUM 150
    ...
    char  fnc_str[FN_SIZE];
    char  fnc_msk[FN_SIZE];
    int   fnc_istr[FNC_NUM];
    int   fnc_ptr1,fnc_ptr2;
...
//  char u_tc[30][5];
  ...
  int u_echo_str4screenstring(int xx,int yy,char *str,int str_size,int st,int vid);
  
// int       deb_tx_locked;

//   SDL_Renderer     *renderer;

//extern int deb_str_has_null(const char *in_str,int in_str_size);
...
static  int  pl_cover_has=0;
static  int  pl_cover_close=0;

static 	int pl_st_play=0;
static 	int pl_st_video=0;

static  int  pl_opts_stt=0;

static 	int pl_thr_v=0;
static 	int pl_thr_s=0;
static 	int pl_thr_r=0;
static 	int pl_thr_a =0;
static 	int pl_thr_a2=0;

static  int  pl_stream_err=0;
static  int  pl_stream_open=0;
static 	int  pl_stream_eos=1;

static  int  pl_frame_num=0;

static 	int    pl_seek_bar_cntr=0;

static 	char      pl_scrn_str1[2001];
static 	char      pl_scrn_str2[8001];

static 	int pl_disp_bar(VideoState *is);
static  int pl_no_support(char *p_str,int p_str_size);
...
#define SR_FFT_BUFFER_SIZE  256
//---------------------------------------------------------------------------
static int sr_show;
static int sr_show_start;
static int sr_show_nodisp;
static int sr_show_init;
static int sr_rate;
static int sr_ch;
static int sr_fft_start;
static int sr_fft_end;
static int sr_river_over;

static int sr_sample_size;

static int  		sr_time_set;
static long long int	sr_time1;
static long long int	sr_time2;
static long long int 	sr_time3;
static long long int 	sr_time5;
static long long int 	sr_total_bytes;

static int sr_river[396][SR_FFT_BUFFER_SIZE/2];
static int sr_river2[150][SR_FFT_BUFFER_SIZE/2];
static int sr_river_ptr;
static int sr_river_mark[396];
static int sr_river_last;
static long long int sr_river_adj;

static int sr_sample_over;
static int sr_sample_over2;

static int sr_sdl_callback_cnt;
...
//#define  DDC_PI  (3.14159265358979323846264338327950288419)
  ...
  static long sr_IsPowerOfTwo(long x);
  static long sr_NumberOfBitsNeeded(long PowerOfTwo);
  static long sr_ReverseBits(long index,long NumBits);
...
//
    ...
    static float sr_Index_to_frequency (long NumSamples,long Index );

    static int   sr_CheckPointer (float *p);

// --------------------------------------------------------------------------

static    float  sr_fft_real_in1[SR_FFT_BUFFER_SIZE];
static    float  sr_fft_real_ou1[SR_FFT_BUFFER_SIZE];
static    float  sr_fft_imag_ou1[SR_FFT_BUFFER_SIZE];
static    float  sr_fft_real_ou2[SR_FFT_BUFFER_SIZE];
static    float  sr_fft_imag_ou2[SR_FFT_BUFFER_SIZE];


static    int     sr_fft_trans_all(VideoState *is,long pcm);
static    int     sr_fft_cx(int chn,int pcm,int mark);
static    int     sr_fft_set_db(long pcm);

//static    int     char2int(char *string,int p1,int p2);
static    int     sr_cc2i(char c1,char c2);
static    void    sr_i2cc(int k,char *cc);

static    float  get_sr_fft_real_in1(long addr);
static    float  get_sr_fft_real_ou1(long addr);
static    float  get_sr_fft_imag_ou1(long addr);
static    float  get_sr_fft_real_ou2(long addr);
static    float  get_sr_fft_imag_ou2(long addr);

static    int    set_sr_fft_real_in1(long addr,float val);
static    int    set_sr_fft_real_ou1(long addr,float val);
static    int    set_sr_fft_imag_ou1(long addr,float val);
static    int    set_sr_fft_real_ou2(long addr,float val);
static    int    set_sr_fft_imag_ou2(long addr,float val);
//--------------------------------------------------------------------------

// ---- river display -------------------------------------------------------
static float   sr_db[60];

static int  sr_river_f[151][118][60][2];

static int  sr_river_f_init;
static int  sr_river_f_init_fail;
static int  sr_river_f_cons(void);
static int  sr_river_f_cons_test(VideoState *cur_stream,int ptr);
static int  sr_river_show(VideoState *cur_stream);
static int  sr_river_show_pause(VideoState *cur_stream);

static int  sr_river_f_test=0;

static char sr_d_buff[7680][4320];
static int  sr_d_return[2];
static int  sr_d_cache[8000][2];
static int  sr_d_cache_ptr;
static int  sr_d_line[2][8000][2];
static int  sr_d_line_ptr[2];
static int  sr_d_line_ptr2[2];
static int  sr_d_line_dot[2][2];

static int  sr_d_init(void);
static int  sr_draw_line(int x1,int y1,int x2,int y2);
static int  sr_draw_line2(int x1,int y1,int x2,int y2);

static int  sr_d_cache_clr(void);

static int sr_draw_line3(int x1,int y1,int x2,int y2);
static int sr_draw_line4_ini(void);
static int sr_draw_line4(int x1,int y1,int x2,int y2,int ptr);
static int sr_draw_line4_get(int ptr);
static int sr_draw_line4_get2(int ptr);
// ------- end of sound river --------------------------------------------
...
#define USER_DOUBLECLICK_EVENT    (SDL_USEREVENT + 3)
...
static void fill_border(int xleft, int ytop, int width, int height, int x, int y, int w, int h)
{
    ...
    fill_rectangle(
                   xleft, ytop,
                   w1, height -/*deb_ch_h*/18*3
                   );
    fill_rectangle(
                   xleft + width - w2, ytop,
                   w2, height -/*deb_ch_h*/18*3 
                   );
    fill_rectangle(
                   xleft + w1, ytop,
                   width - w1 - w2, h1
    ...
    fill_rectangle(
                   xleft + w1, ytop + height - h2 -/*deb_ch_h*/18*3 ,
                   width - w1 - w2, h2
                   );
}
...
static int video_open(VideoState *is)
{
    ...
    screen_w = screen_width  = is->width  = w;
    screen_h = screen_height = is->height = h;
...
static int deb_video_open_again(VideoState *is, int force_set_video_mode)//daipozhi modified
{
    ...
    screen_width  = is->width  = screen_w ;
    screen_height = is->height = screen_h ;

    SDL_SetWindowTitle(window, ftree_dir_buffer);
...
static void update_sample_display(VideoState *is, short *samples, int samples_size)
{
    ...
    // daipozhi modified for sound river
    if (sr_show_start==1) sr_total_bytes=sr_total_bytes+samples_size;
#if DPZ_DEBUG1
    k=samples_size/(2*sr_ch);
    ...
    while (size > 0) {
        ...
        //daipozhi modified
        len = /*SAMPLE_ARRAY_SIZE*/ sr_sample_size - is->sample_array_index;
        ...
        //daipozhi modified
        if (is->sample_array_index >= /*SAMPLE_ARRAY_SIZE*/ sr_sample_size )
        {
	...
	if (sr_show_init==0)
	{
	    ...
	    sr_fft_deb_ptr2=0;
	    sr_fft_deb_ptr3=0;
	    sr_sdl_callback_cnt=0;
	    sr_fft_deb_chn=0;
#endif
	    for (l=0;l<396;l++)
	    {
		for (m=0;m<SR_FFT_BUFFER_SIZE/2;m++) sr_river[l][m]=0;
		sr_river_mark[l]=0;
	    }

	    for (l=0;l<150;l++)
	    {
		for (m=0;m<SR_FFT_BUFFER_SIZE/2;m++) sr_river2[l][m]=0;
	    }


	    l=is->sample_array_index;
	    l=(SR_FFT_BUFFER_SIZE*sr_ch)*(l/(SR_FFT_BUFFER_SIZE*sr_ch));		// fft start and end
	    m=l;
	    sr_fft_start=l;
	    sr_fft_end=m;
	...
	else
	{
	    ...
	    if (sr_fft_end<=is->sample_array_index)	// sample position of now
	    {
		l=sr_fft_end;		// fft start and end
		m=(SR_FFT_BUFFER_SIZE*sr_ch)*(is->sample_array_index/(SR_FFT_BUFFER_SIZE*sr_ch));
		sr_fft_start=l;
		sr_fft_end=m;
	    }
	    else
		...
		{
			sr_fft_start=sr_fft_end;
		}
	    }

	    if (sr_fft_start==sr_fft_end) return;
	    if ((sr_fft_start==sr_sample_size)&&(sr_fft_end==0)) return;
#if DPZ_DEBUG1
	    sprintf(m711_str1,"next,fft start=%d,fft end=%d,samp index=%d,",sr_fft_start,sr_fft_end,is->sample_array_index);
            deb_record(m711_str1);
#endif
	}

        // start fft
	sr_fft_trans_all(is,sr_rate);
    }
           ...
           if (audio_size < 0) {
               ...
               else if (sr_rate==48000)
               {
                 ...
                 while (1)
                 {
                     ...
                     if (  160*rept-start+m  <  j/(sr_ch*2)  )
                     {

                       for (n=0;n<sr_ch;n++)
                       {

                         deb_resam_buff2[(i*sr_ch+n)*2+0]=deb_resam_buff1[( (160*rept-start+m)*sr_ch+n )*2+0];
                         deb_resam_buff2[(i*sr_ch+n)*2+1]=deb_resam_buff1[( (160*rept-start+m)*sr_ch+n )*2+1];
                         if (size<i) size=i;
                       }
                 ...
                 for (i=  ((160*rept-start+m)*sr_ch+2)*2  ;i<j;i++) deb_resam_buff3[i-  ((160*rept-start+m)*sr_ch+2)*2  ]=deb_resam_buff1[i];
                 deb_resam_buff3_ptr=j-  ((160*rept-start+m)*sr_ch+2)*2  ;
                 size=(size+1)*sr_ch*2;
               }
               else if (sr_rate==32000)
               {
                 size=0;
                 i=0;
                 rept=0;
                 ...
                 while (1)
                 {
                     ...
                     if (  320*rept-start+m  <  j/(sr_ch*2)  )
                     {

                       for (n=0;n<sr_ch;n++)
                       {

                         deb_resam_buff2[(i*sr_ch+n)*2+0]=deb_resam_buff1[( (320*rept-start+m)*sr_ch+n )*2+0];
                         deb_resam_buff2[(i*sr_ch+n)*2+1]=deb_resam_buff1[( (320*rept-start+m)*sr_ch+n )*2+1];
                         if (size<i) size=i;
                 ...
                 for (i=  ((320*rept-start+m)*sr_ch+2)*2  ;i<j;i++) deb_resam_buff3[i-  ((320*rept-start+m)*sr_ch+2)*2  ]=deb_resam_buff1[i];
                 deb_resam_buff3_ptr=j-  ((320*rept-start+m)*sr_ch+2)*2  ;
                 size=(size+1)*sr_ch*2;
               }
               else size=j;
...
// daipozhi modified for sound river
	...
	while (len2<len1)
	{
	   if (sr_fft_deb_ptr1+len3<SR_FFT_BUFFER_SIZE*2*sr_ch)  
           {
	     if ( sr_sdl_callback_cnt>=SR_FFT_BUFFER_SIZE*2*sr_ch*2 ) 
	     {
        	if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
		{
            		memcpy(stream2, (uint8_t *)(sr_fft_deb[sr_fft_deb_ptr3]) + sr_fft_deb_ptr1+len3, 1);
		}
        	else 
		{
            		memset(stream2, 0, 1);
            		if (!is->muted && is->audio_buf)
                	SDL_MixAudioFormat(stream2, (uint8_t *)(sr_fft_deb[sr_fft_deb_ptr3]) + sr_fft_deb_ptr1+len3, AUDIO_S16SYS, 1, is->audio_volume);
       		}
	   ...
	   {
		sr_fft_deb_ptr3++;
		if (sr_fft_deb_ptr3>=4) sr_fft_deb_ptr3=0;
	...
	sr_fft_deb_ptr1=sr_fft_deb_ptr1+len3;
#else
        if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
    ...
    // daipozhi for sound river
    if ( sr_sdl_callback_cnt<SR_FFT_BUFFER_SIZE*2*sr_ch*2 ) sr_sdl_callback_cnt=sr_sdl_callback_cnt+len4;
...
static int audio_open(void *opaque, AVChannelLayout *wanted_channel_layout, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    sr_sample_size=(SAMPLE_ARRAY_SIZE/(spec.channels*SR_FFT_BUFFER_SIZE))*spec.channels*SR_FFT_BUFFER_SIZE;

#if DPZ_DEBUG1
    sprintf(m702_str1,"init show=%d,rate=%d,ch=%d,",sr_show,sr_rate,sr_ch);
                               ...
                               const AVInputFormat *iformat,int step)
{
    ...
    /* start video dstream_open_isplay */
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    ...
    while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) {
        ...
        remaining_time = REFRESH_RATE;
                ...
                // daipozhi modified for sound river
		if ((sr_show_start==1)&&(sr_time_set==0)&&(pl_thr_a))
		{
			gettimeofday(&tv,NULL/*,&tz*/);
			sr_time1=tv.tv_sec;
			sr_time2=tv.tv_usec;
			sr_time3=sr_time1*1000000+sr_time2;
			sr_time_set=1;
		}
		...
		if (pl_st_play==1)
		{
			if (pl_frame_num<50)  // at opened stream's first 0.5s to do nothing, to avoid error
			{
				pl_frame_num++;
			}
			else
			{
				if ((sr_show==1)&&(sr_show_start==1)&&(sr_show_nodisp==0))
				{
				  g_paint_ini();
				  sr_river_show(is);  // daipozhi for sound river
				  //deb_m_ref=1;
				}

				if ((pl_thr_v)&&(pl_thr_a)&&(pl_thr_a2)&&(pl_thr_r))
				{
					if (is->video_st)
					{
        ...
        if (is->show_mode != SHOW_MODE_NONE && (!is->paused || is->force_refresh))
        {
            video_refresh(is, &remaining_time);
				...
				pl_seek_bar_cntr++;   //daipozhi modified
				
				if (pl_seek_bar_cntr>=50) //update seek bar in every 0.5s
				{
					pl_seek_bar_cntr=0;
					
					if (is->audio_st && is->show_mode != SHOW_MODE_VIDEO)
					{
						if ((pl_thr_a)&&(pl_thr_a2)&&(pl_thr_r))  // audio without image
						{
							/*if ((pl_st_play==1)&&(is->show_mode == SHOW_MODE_VIDEO)&&(pl_cover_close==0))
							      video_image_display(is);
							else*/ if ((sr_show==1)&&(sr_show_start==1)&&(sr_show_nodisp==0))
							{
							  g_paint_ini();
							  sr_river_show_pause(is);
							  //deb_m_ref=1;
							}
							else
							{
							...
							//its purpose is pl_disp_bar()
						}
					}
					else if (is->video_st && is->show_mode == SHOW_MODE_VIDEO)
					{
						if ((pl_thr_v)&&(pl_thr_a)&&(pl_thr_a2)&&(pl_thr_r))  //video or audio with picture
						{
							if ((pl_st_play==1)&&(is->show_mode == SHOW_MODE_VIDEO)&&(pl_cover_close==0))
							{
							    video_image_display(is);
    ...
    {
        //printf("into double click,\n\n");
        
	gettimeofday(&tv,NULL/*,&tz*/);
	ftree_time1=tv.tv_sec;
	ftree_time2=tv.tv_usec;
	ftree_time3=ftree_time1*1000000+ftree_time2;
	ftree_time_set1=1;
	
	if (ftree_time_set2==1)  //before event is SDL_MOUSEBUTTONDOWN
	{
	    ftree_time5=ftree_time3-ftree_time4;

	    //printf("double click time=%lld,\n\n",ftree_time5);
	        
	    if (ftree_time5<300000)
	    {
	        event2.type = USER_DOUBLECLICK_EVENT;
	        event2.user.code = 2;
	        event2.user.data1 = NULL;
	        event2.user.data2 = NULL;
	        
	        //event2.button.x = (*event).button.x;
	        //event2.button.y = (*event).button.y;
	        
	        event2.user.data1 = is;
	        
	        SDL_PushEvent(&event2);

	        ftree_time_x = (*event).button.x;
	        ftree_time_y = (*event).button.y;
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
	    ...
	    pl_disp_bar(cur_stream);
	    g_paint_dirview();
	    
	    g_def_proc(event,vid);
		...
		sr_river_f_cons_test(cur_stream,sr_river_f_test);
		sr_river_f_test++;
...
#if DPZ_DEBUG2
		sr_fft_deb_chn++;
		if (sr_fft_deb_chn>=3) sr_fft_deb_chn=0;
#endif
                break;
		...
		g_mouse_down=0;
		     ...
		     if ((pl_st_play==1)&&(cur_stream->show_mode == SHOW_MODE_VIDEO)&&(pl_cover_close==0))
		     {
		     ...
		     {
		       g_paint_ini();
		       sr_river_show_pause(cur_stream);
		     }
		     else
		     {
		       g_paint_ini();
	    ...
	    g_mouse_down=1;
            ...
            av_log(NULL, AV_LOG_INFO,"event mouse button down,x=%d,y=%d,\n",event.button.x,event.button.y);
	    ...
	    {
	      if ((pl_cover_has==1)&&(pl_cover_close==0))
	      {
		pl_cover_close=1;
	        g_paint_ini();
		pl_disp_bar(cur_stream);
		...
		break;
	      ...
	      else if ((sr_show_start==1)&&(sr_show_nodisp==0))
	      {
		  sr_show_nodisp=1;
		  sr_show_start=0;
	          g_paint_ini();
	    ...
	    if ((g_detect_ptr1==1)&&(g_detect_ptr2==0)&&(g_detect_ptr3==1))    // in play list
	    {
		  ...
		  while ((ftree_filenamebuff_subline[ftree_filenamebuff_n+n2]==1)&&(ftree_filenamebuff_n+n2>0))
		  {
		    n2--;
		  }

	          if (ftree_filenamebuff_n+n2>=ftree_filenamebuff_ptr) // more than filenamecnt 
		  {
                    break;
		  }

	          if (ftree_filenamebuff_n+n2>=MAX_FILE_NUM) // more than filenamecnt 
		  {
                    break;
		  }

	          if (ftree_filenamebuff_n+n2<0)
		  {
                    break;
		  }
                  ...
                  if (g_key_ctrl==1) 
                  {
                    if (ftree_filenamebuff_select[ftree_filenamebuff_n+n2]==1) ftree_filenamebuff_select[ftree_filenamebuff_n+n2]=0;
                    else                                                       ftree_filenamebuff_select[ftree_filenamebuff_n+n2]=1;
                  }
                  else if (g_key_shift==1)
                  {
                    j=ftree_filenamebuff_n+n2;
                    n=j;
                    o=j;
                    m=ftree_get_space(j);
                    ...
                    while ((ftree_get_space(j)==m)&&(  (ftree_filenamebuff_select[j]==0)&&(ftree_filenamebuff_subline[j]==0) ||
                                                       (ftree_filenamebuff_subline[j]==1)                                       )  )
                    {
                      o=j;
                      j--;
                    }
                    
                    if ((ftree_get_space(j)==m)&&(ftree_filenamebuff_select[j]==1))
                    {
                      for (p=o;p<=n;p++)
                      {
                        ftree_filenamebuff_select[p]=1;
                      }
                    }
                    else
                    {
                      ftree_filenamebuff_select[ftree_filenamebuff_n+n2]=1;
                    }
                  }
                  else
                  {
                    ...
                    while(  (j<ftree_filenamebuff_ptr) &&
                            (j<MAX_FILE_NUM)           &&
                            (j>=0)                         )
                     ftree_filenamebuff_select[j++]=0;
                  ...
                  while(  (j<ftree_filenamebuff_ptr) &&
                          (j<MAX_FILE_NUM)           &&
                          (j>=0)                         )
                     if (ftree_filenamebuff_select[j++]==1) k++;
                             
                 sprintf(str2,"%d Object(s) Selected",k);
		 g_set_label_text(1,str2,FN_SIZE);
	    ...
	    if ((g_detect_ptr1==1)&&(g_detect_ptr2==0))   //in border line
	    {
		    if (g_detect_ptr3==2)
		    {
		      ...
		      n3=g_dirview_posi[g_detect_ptr2][3]/(14+2);
		      
		      if (ftree_filenamebuff_n>=n3) ftree_filenamebuff_n=ftree_filenamebuff_n-n3;
		      else ftree_filenamebuff_n=0;
		    ...
		    else if (g_detect_ptr3==4) // down page
		    {
		      ...
		      if (ftree_filenamebuff_n+n3+n3<ftree_filenamebuff_ptr) ftree_filenamebuff_n=ftree_filenamebuff_n+n3;
		      else
		      {
			ftree_filenamebuff_n=ftree_filenamebuff_ptr-n3;
			if (ftree_filenamebuff_n<0) ftree_filenamebuff_n=0;
		      }
		...
		if ((pl_cover_has==1)&&(pl_cover_close==0)) // status switch, video,river,file name
		{
		  if (sr_show==1)
		  {
		    pl_cover_close=1;
		...
		else if ((pl_cover_has==1)&&(pl_cover_close==1))
		{
		  g_paint_ini();
		  pl_disp_bar(cur_stream);
		  g_paint_dirview();
    ...
    g_create_scrollbar((cur_stream->width-310)/2+45+5+200+5+150+200,1+18*2,18,18*5,0,255,0,0,1);
    g_set_scrollbar_value_max(3,100);
    ...
    // test end 
	    ...
	    if ((g_detect_ptr1==2)&&(g_detect_ptr2==1)) //volume bar
	    {
	        deb_m_vol=SDL_MIX_MAXVOLUME*(g_detect_ptr3)/180;
                g_set_scrollbar_value_now(1,deb_m_vol);
		if (pl_st_play==1) cur_stream->audio_volume=deb_m_vol;
	    ...
	    if (g_detect_ptr1==7) //menu
	    {
	        if (g_menu_item_haschild[g_detect_ptr4][g_menu_paint_n1[g_detect_ptr2]+g_detect_ptr3]==1)
	        {
	            g_menu_paint_prepare(xx,
	                                 yy,
	                                 g_detect_ptr4,
	                                 g_menu_item_self[g_detect_ptr4][g_menu_paint_n1[g_detect_ptr2]+g_detect_ptr3]);
	        }
	        else
	        {
	            printf("menu:%s,\n\n",g_menu_item_self[g_detect_ptr4][g_menu_paint_n1[g_detect_ptr2]+g_detect_ptr3]);
	            ...
	            pl_play_selected_list_ptr=0;
                    ...
                    while(    (j<ftree_filenamebuff_ptr) &&
                              ...
                              (j>=0)                         )
                    {
                       if ((ftree_filenamebuff_subline[j]==0)&&(ftree_filenamebuff_select[j]==1))
                       {
			  str3[0]=0;
			  strcpy(str3,ftree_filenamebuff[j]);
			  l=j+1;
		  
			  while ((ftree_filenamebuff_subline[l]==1)&&(l<ftree_filenamebuff_ptr))
			  {
			    if (strlen(str3)+strlen(ftree_filenamebuff[l])>=FN_SIZE) break;
		    
			    strcat(str3,ftree_filenamebuff[l]);
			    l++;
			  }
		  
		  	  ftree_get_path(j);
        	          sc1=ftree_filenamebuff_type[j];
        	          
        	          strcpy(pl_play_selected_list[pl_play_selected_list_ptr],ftree_dir_buffer);
        	          pl_play_selected_list_type[pl_play_selected_list_ptr]=sc1;
        	          
        	          pl_play_selected_list_ptr++;
                       }
	           ...
	           pl_play_selected=1;
	           if (strcmp(g_menu_item_self[g_detect_ptr4][g_menu_paint_n1[g_detect_ptr2]+g_detect_ptr3],"12")==0) pl_play_selected_repeatedly=1;
	           else pl_play_selected_repeatedly=0;
	           pl_play_selected_list_play=0;
	           


                   sc1=pl_play_selected_list_type[0];
		   ...
		   if (sc1!=0)        // not dir not empty
		   {
		     ...
		     if (pl_no_support(pl_play_selected_list[0],FN_SIZE)==1) break;
		     ...
		     strcpy(ftree_dir_buffer,pl_play_selected_list[0]);
		     ...
		     if (deb_str_has_null(pl_play_selected_list[0],FN_SIZE)!=1) break;
		     if ((strlen(pl_play_selected_list[0])>=FN_SIZE)||((int)strlen(pl_play_selected_list[0])<0)) break;
		     strcpy(ftree_currentpath,pl_play_selected_list[0]);

                     pl_stream_err=1;
                     pl_stream_open=1;
                    
		     stream_open(pl_play_selected_list[0], file_iformat,2);
	    ...
	    else if (event.button.button==SDL_BUTTON_RIGHT)
	    {
            ...
            av_log(NULL, AV_LOG_INFO,"event mouse right button down,x=%d,y=%d,\n",event.button.x,event.button.y);
	        ...
	        if (ftree_filenamebuff_n+n2<0)
		{
                ...
                if (ftree_filenamebuff_select[ftree_filenamebuff_n+n2]==1) 
                {
                }
                else 
                {
	        ...
	        g_menu_paint_prepare(xx,yy,1,str4);
        ...
        case USER_DOUBLECLICK_EVENT:
            
	    xx=ftree_time_x;
	    yy=ftree_time_y;

            printf("event mouse button double click,x=%d,y=%d,\n\n",xx,yy);
            
	    g_detect_click(xx,yy);
	    
	    //printf("position=%d,%d,%d,\n",g_detect_ptr1,g_detect_ptr2,g_detect_ptr3);
	    ...
	    if ((g_detect_ptr1==1)&&(g_detect_ptr2==0)&&(g_detect_ptr3==1))    // in play list
	    {
		  ...
		  str3[0]=0;
		  strcpy(str3,ftree_filenamebuff[ftree_filenamebuff_n+n2]);
		  l=ftree_filenamebuff_n+n2+1;
		  ...
		  while ((ftree_filenamebuff_subline[l]==1)&&(l<ftree_filenamebuff_ptr))
		  {
		    ...
		    l++;
                  ...
                  sc1=ftree_filenamebuff_type[ftree_filenamebuff_n+n2];
		  ...
		  if (sc1!=0)        // not dir not empty
		  {
		    ...
		    if (pl_no_support(str3,FN_SIZE)==1) break;

		    ftree_get_path(ftree_filenamebuff_n+n2);
		    ...
		    ftree_filenameplay=ftree_filenamebuff_n+n2;
		    ...
		    if (deb_str_has_null(ftree_dir_buffer,FN_SIZE)!=1) break;
		    if ((strlen(ftree_dir_buffer)>=FN_SIZE)||((int)strlen(ftree_dir_buffer)<0)) break;
		    strcpy(ftree_currentpath,ftree_dir_buffer);

                    pl_stream_err=1;
                    pl_stream_open=1;
                    
		    stream_open(ftree_dir_buffer, file_iformat,2);
		  ...
		  else //dir
		  {
		    ftree_get_path(ftree_filenamebuff_n+n2);
                    ...
                    s_dir_opened=ftree_dir_opened(ftree_filenamebuff_n+n2);
                    
                    if (s_dir_opened==0)
                    {
                      //printf("chdir\n");
                      
		      i=chdir(ftree_dir_buffer);
                      if (i!=0) break;

		      i=(long long int)getcwd(ftree_currentpath,FN_SIZE);
                      if (i==0) break;

                      if (ftree_cmp_dir(ftree_dir_buffer,ftree_currentpath)==0)
		      {
			ftree_read_dir();
        	        ftree_dir_add_after(ftree_filenamebuff_n+n2);
		        g_paint_ini();
			pl_disp_bar(cur_stream);
			g_paint_dirview();
			...
			//printf("add after\n");
		        ...
		        break;
                    ...
                    {
                      ftree_dir_remove_after(ftree_filenamebuff_n+n2);
		      g_paint_ini();
		      pl_disp_bar(cur_stream);
		      g_paint_dirview();
        ...
        case SDL_WINDOWEVENT:
		    ...
		    //daipozhi modified
	            ...
	            //daipozhi modified
	            sr_river_f_init=0; //daipozhi modified for sound river
                    
                    // for no testing of mini GUI lib
                    
		    g_move_dirview(0,0,0,cur_stream->width,cur_stream->height-18*3-9);
		    ...
		    g_move_label(1,5,cur_stream->height-18*3+1,cur_stream->width-5-5,16);
		    ...
		    g_move_button(4,(cur_stream->width-310)/2+45+5+200+5+150,1     ,55,16);
		    g_move_button(5,(cur_stream->width-310)/2+45+5+200+5+150,1+18  ,55,16);
		    g_move_button(6,(cur_stream->width-310)/2+45+5+200+5+150,1+18*2,55,16);

		    g_move_scrollbar(2,(cur_stream->width-310)/2+45+5+200+5+150,1+18*3,180,16);
		    g_move_scrollbar(3,(cur_stream->width-310)/2+45+5+200+5+150+200,1+18*2,18,18*5);

		    g_move_label(1,(cur_stream->width-310)/2+45+5+200+5+150,1+18*4,150,16);
		    */
                    // test end
                    
             
             	     ftree_conv_dir();   //resize dir view
	    ...
	    while ((ftree_filenamebuff_subline[ftree_filenameplay]==1)&&(ftree_filenameplay<ftree_filenamebuff_ptr))
	    {
	      ftree_filenameplay++;
	    }
	    ...
	    g_def_proc(event,vid);
            ...
            if (ftree_filenameplay>=ftree_filenamebuff_ptr) // more than filenamecnt 
	    {
	      ftree_filenameplay=ftree_filenamebuff_ptr-1;
              break;
	    }
            if (ftree_filenameplay>=MAX_FILE_NUM) // more than MAX_FILE_NUM
	    {
	      ftree_filenameplay=MAX_FILE_NUM-1;
              break;
	    }
            if (ftree_filenameplay<0) 
	    {
	      ftree_filenameplay=0;
              break;
	    }

	    str3[0]=0;
	    strcpy(str3,ftree_filenamebuff[ftree_filenameplay]);
	    l=ftree_filenameplay+1;
	    ...
	    while ((ftree_filenamebuff_subline[l]==1)&&(l<ftree_filenamebuff_ptr))
	    {
	      ...
	      l++;
            ...
            sc1=ftree_filenamebuff_type[ftree_filenameplay];  //daipozhi modified for audio
            if (sc1==2) break; // comment line
            if (sc1==3) break; // empty line
	    if (sc1!=0)        // not dir not empty
	    {
	      ...
	      ftree_get_path(ftree_filenameplay);
...
*/
	  ...
	  else
	  {
	    ...
	    g_def_proc(event,vid);
            ...
            if (pl_play_selected_list_play>=pl_play_selected_list_ptr) // more than filenamecnt 
	    {
	      if (pl_play_selected_repeatedly==1) pl_play_selected_list_play=0;
              else break;
	    }

            sc1=pl_play_selected_list_type[pl_play_selected_list_play];  //daipozhi modified for audio
            if (sc1==2) break; // comment line
            if (sc1==3) break; // empty line
	    if (sc1!=0)        // not dir not empty
	    {
	      if (pl_no_support(pl_play_selected_list[pl_play_selected_list_play],FN_SIZE)==1) break;
	      
	      strcpy(ftree_dir_buffer,pl_play_selected_list[pl_play_selected_list_play]);
	      ...
	      if (deb_str_has_null(pl_play_selected_list[pl_play_selected_list_play],FN_SIZE)!=1) break;
	      if ((strlen(pl_play_selected_list[pl_play_selected_list_play])>=FN_SIZE)||((int)strlen(pl_play_selected_list[pl_play_selected_list_play])<0)) break;
	      strcpy(ftree_currentpath,pl_play_selected_list[pl_play_selected_list_play]);

              pl_stream_err=1;
              pl_stream_open=1;

	      stream_open(pl_play_selected_list[pl_play_selected_list_play], file_iformat,2);
...
*/
	...
	case SDL_MOUSEWHEEL:
	        ...
	        vid=0;
                ...
                av_log(NULL, AV_LOG_INFO,"event mouse wheel event,\n");
                  
		if ((sr_show==1)&&(sr_show_start==1)&&(sr_show_nodisp==0)) break;

		if (cur_stream->video_st && cur_stream->show_mode == SHOW_MODE_VIDEO)
			if (pl_cover_close==0) break;

	        if (event.wheel.y > 0) // wheel scroll up
	        {
		    if (ftree_filenamebuff_n>=5) ftree_filenamebuff_n=ftree_filenamebuff_n-5;
		    else ftree_filenamebuff_n=0;
		    ...
		    g_def_proc_paint(vid);
		    
		    break;
	        }
	        else if (event.wheel.y < 0) // wheel scroll down
	        {
		    n3=(cur_stream->height)/(deb_ch_h+2)-2-2;
		    
		    if (ftree_filenamebuff_n+n3+5<ftree_filenamebuff_ptr) ftree_filenamebuff_n=ftree_filenamebuff_n+5;
		    else
		    {
		      ftree_filenamebuff_n=ftree_filenamebuff_ptr-n3;
		      if (ftree_filenamebuff_n<0) ftree_filenamebuff_n=0;
...
int main(void/*int argc, char **argv*/)
{
    ...
    ftree_read_dir_ini();  //daipozhi modified  
    ...
    g_create_dirview(0,0,is->width,is->height-18*3-9);
    ...
    */
    ...
    strcpy(str1,"");
    g_create_label(5,is->height-18*3+1,600,16,0,240,240,240,str1,5);
    ...
    g_create_scrollbar((is->width-310)/2+45+5+200+5+150+200,1+18*2,18,18*5,0,255,0,0,1);
    g_set_scrollbar_value_max(3,100);
...
// daipozhi modified
/*
static int ftree_cmp_dir(char *buffer1,char *buffer2)
{ 
  int  i,j,k;
  char c1,c2;
...
static int pl_disp_bar(VideoState *cur_stream2)
{
  int   uns, uhh, umm, uss;
  int   tns, thh, tmm, tss;
  ...
  if (  (  (cur_stream2->show_mode != SHOW_MODE_VIDEO)  &&  ((pl_thr_r!=1)||(pl_thr_a!=1)||(pl_thr_a2!=1)||(!cur_stream2->audio_st))  )                 ||
        (  (cur_stream2->show_mode == SHOW_MODE_VIDEO)  &&  ((pl_thr_r!=1)||(pl_thr_a!=1)||(pl_thr_a2!=1)||(pl_thr_v!=1)||(!cur_stream2->video_st))  ) ||
        (  (seek_by_bytes || cur_stream2->ic->duration<=0)   )   )
  {
    ...
    snprintf(pl_scrn_str1,300," %2d:%2d:%2d / %2d:%2d:%2d ",uhh,umm,uss,thh,tmm,tss);

    g_set_label_text(0,pl_scrn_str1,300);
    
    strcpy(pl_scrn_str1,"Play");
    
    g_set_button_text(0,pl_scrn_str1,300);
  ...
  else
  {
    ...
    if ((pl_st_play==1)&&(cur_stream2->paused==0)) 
    {
      strcpy(pl_scrn_str1,"Pause");
    }
    else
    {
      strcpy(pl_scrn_str1,"Play");
	...
	if (sr_show==1)
        {
	  strcpy(pl_scrn_str2,"River On");
	}
	else
	{
	  strcpy(pl_scrn_str2,"Video Off");
	}
      }
      else
      {
    ...
    g_set_button_text(1,pl_scrn_str2,300);
...
/*
 int deb_str_has_null(char *str,int str_size)
{
	int i;
...
static int sr_fft_trans_all(VideoState *is,long pcm)
{
  ...
  while(1)
  {
    if (((sr_sample_over2==0)&&(l<sr_fft_end))||
	((sr_sample_over2==1)&&(l<sr_sample_size)))
    {
        // trans for left and right channel to many
        for (j=0;j<sr_ch;j=j+sr_ch)
	{
          for (m=0;m<SR_FFT_BUFFER_SIZE;m++)
	  {
		...
		*/
		...
		if ((p<0)||(p>=sr_sample_size)) continue;

		n=is->sample_array[p];
		
		//n=n/3;   // reduce volumn , prevent 16bit int overflow

	        set_sr_fft_real_in1(m,(float)n);
	  }

	  // transform to freq
	  i=sr_fft_float(SR_FFT_BUFFER_SIZE,sr_fft_real_in1,sr_fft_real_ou1,sr_fft_imag_ou1);
	  if (i!=0) return(1);

	  // store to buffer for multi get
	  for (m=0;m<SR_FFT_BUFFER_SIZE;m++)
	  {
		d1=get_sr_fft_real_ou1(m);
        	set_sr_fft_real_ou2(m,d1);

		d1=get_sr_fft_imag_ou1(m);
        	set_sr_fft_imag_ou2(m,d1);
	  }
	  ...
	  // 1 chn 
          ...
          if (m!=0) return(1);
#else
	  for (i=0;i<SR_FFT_BUFFER_SIZE/2;i++)
	  {
	    // 1st to 256th chn 
            m=sr_fft_cx(i,pcm,l);
            if (m!=0) return(1);
	...
	if (sr_river_ptr>=396) //ring buffer
	{
	  sr_river_ptr=0;
	  sr_river_over=1;
	}

	l=l+SR_FFT_BUFFER_SIZE*sr_ch;

	if (sr_sample_over2==1)  //ring buffer
	{
	  if (l>=sr_sample_size)
	  {
	    l=l-sr_sample_size;
...
static int sr_fft_cx(int chn,int pcm,int mark)
{
  ...
  if ((chn<0)||(chn>=SR_FFT_BUFFER_SIZE/2)) return(0);
  ...
  for (m=0;m<SR_FFT_BUFFER_SIZE;m++)
  {
    f1=(float)m/(float)SR_FFT_BUFFER_SIZE;

    if (sr_fft_deb_chn==0) //to playback high freq,low freq,middle freq audio,to sure fft is ok,need click '[river on]',set DPZ_DEBUG2 to 1,press key 'm' to switch chanel
    {
      if (((f1>=0    )&&(f1< 0.017))||
          ((f1> 0.983)&&(f1<=1    )))  // low freq
      {
	  d1=get_sr_fft_real_ou2(m);
	  set_sr_fft_real_in1(m,d1);
	  d1=get_sr_fft_imag_ou2(m);
	  set_sr_fft_imag_ou1(m,d1);
      }
      else
      {
	  set_sr_fft_real_in1(m,0);
          set_sr_fft_imag_ou1(m,0);
      }
    }
  ...
  i=sr_ifft_float(SR_FFT_BUFFER_SIZE,sr_fft_real_in1,sr_fft_real_ou1,sr_fft_imag_ou1);
  if (i!=0) return(1);

  // play back
  for (m=0;m<SR_FFT_BUFFER_SIZE;m++)
  {
    f2=get_sr_fft_real_ou1(m);
    ...
    for (p=0;p<sr_ch;p++)
    {
	if (p==0)  sr_fft_deb[sr_fft_deb_ptr2][m*sr_ch+p]=j; // left channel
	else       sr_fft_deb[sr_fft_deb_ptr2][m*sr_ch+p]=0; // other channel
    }
  }

  sr_fft_deb_ptr2++;
  if (sr_fft_deb_ptr2>=4) sr_fft_deb_ptr2=0;
  ...
  if (chn<118) // if freq more than 20khz don't show
  {
    ...
    for (m=0;m<SR_FFT_BUFFER_SIZE;m++)
    {
      //f1=pcm*((float)m/(float)SR_FFT_BUFFER_SIZE);

      //if (((f1>=sr_frq[chn])&&(f1<sr_frq[chn+1]))||
      //    ((f1>pcm-sr_frq[chn+1])&&(f1<=pcm-sr_frq[chn])))  // ??? > >= < <=

      //if (((m>=frq1)&&(m<frq2))||
      //    ((m>=SR_FFT_BUFFER_SIZE-frq2)&&(m<SR_FFT_BUFFER_SIZE-frq1)))

      if ((m-0==chn)||(SR_FFT_BUFFER_SIZE-1-m==chn))
      {
	d1=get_sr_fft_real_ou2(m);
	set_sr_fft_real_in1(m,d1);
    ...
    lp=lp/(float)SR_FFT_BUFFER_SIZE;
    if (lp<0) lp=0;
  ...
  if ((sr_river_ptr<0)||(sr_river_ptr>=396)) return(0);

  sr_river[sr_river_ptr][chn]=n;
  sr_river_mark[sr_river_ptr]=mark;

  #if DPZ_DEBUG1
  //sprintf(m601_str1,"fft cx,river_ptr=%d,chn=%d,val=%d,mark=%d,",sr_river_ptr,chn,n,mark);
  //deb_record(m601_str1);
  #endif
...
static int sr_fft_set_db(long pcm)
{
  ...
  for (i=0;i<60;i++)
  {
    sprintf(m602_str1,"init db i=%d,db=%10.2f,",i,sr_db[i]);
...
static float  get_sr_fft_real_in1(long addr)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	return(sr_fft_real_in1[addr]);
}
static float  get_sr_fft_real_ou1(long addr)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	return(sr_fft_real_ou1[addr]);
}
static float  get_sr_fft_imag_ou1(long addr)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	return(sr_fft_imag_ou1[addr]);
}
static float  get_sr_fft_real_ou2(long addr)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	return(sr_fft_real_ou2[addr]);
}
static float  get_sr_fft_imag_ou2(long addr)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	return(sr_fft_imag_ou2[addr]);
}

static int  set_sr_fft_real_in1(long addr,float val)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	sr_fft_real_in1[addr]=val;
	return(0);
}
static int  set_sr_fft_real_ou1(long addr,float val)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	sr_fft_real_ou1[addr]=val;
	return(0);
}
static int  set_sr_fft_imag_ou1(long addr,float val)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	sr_fft_imag_ou1[addr]=val;
	return(0);
}
static int  set_sr_fft_real_ou2(long addr,float val)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	sr_fft_real_ou2[addr]=val;
	return(0);
}
static int  set_sr_fft_imag_ou2(long addr,float val)
{
	if ((addr<0)||(addr>=SR_FFT_BUFFER_SIZE)) return(0);
	sr_fft_imag_ou2[addr]=val;
	return(0);
}
...
static int sr_fft_float( long    NumSamples,
			     float *RealIn,
		             float *RealOut,
		             float *ImagOut )
{
   ...
   if ( !sr_IsPowerOfTwo(NumSamples) ) return(1);

   if (sr_CheckPointer(RealIn )) return(1);
   if (sr_CheckPointer(RealOut)) return(1);
   if (sr_CheckPointer(ImagOut)) return(1);

   NumBits=sr_NumberOfBitsNeeded(NumSamples);
...
static int sr_ifft_float(long     NumSamples,
			     float  *RealIn,
		             float  *RealOut,
		             float  *ImagOut )
{
...
so   d2=(h-14*2-7)/151
*/  
  d2=((float)screen_h-18*3-9-7-14-10-10)/151;
  ...
  for (i=0;i<151;i++)
  {
    sr_river_f[i][118-1][0][0]=x2;
    sr_river_f[i][118-1][0][1]=y2;
  ...
  {
    x2=sr_river_f[i][118-1][0][0];
    y2=sr_river_f[i][118-1][0][1];
    ...
    for (j=118-1-1;j>=0;j--)
    {
      ...
      l=sr_draw_line2(x3,y3,x4,y4);
      if (l!=0) return(1);

      sr_river_f[i][j][0][0]=sr_d_return[0];
      sr_river_f[i][j][0][1]=sr_d_return[1];
    }
  }
  ...
  for (i=0;i<151;i++)    // to 3d part 1
  {
    for (j=118-1;j>=0;j--)
    {
      x2=sr_river_f[i][j][0][0];
      y2=sr_river_f[i][j][0][1];
      ...
      if (j==118-1)  // front wall
      {
        for (k=1;k<60;k++)
        {
          ...
          sr_river_f[i][j][k][0]=x3;
          sr_river_f[i][j][k][1]=y3;
        }
      }
      else
      {
	  ...
	  sr_d_cache_clr();
	  l=sr_draw_line(x3,y3,x4,y4);
          if (l!=0) return(1);

          for (k=1;k<60;k++)
          {
            x5=sr_river_f[i][118-1][k][0];
            y5=sr_river_f[i][118-1][k][1];
            ...
            l=sr_draw_line2(x6,y6,x5,y5);
            if (l!=0) return(1);

            sr_river_f[i][j][k][0]=sr_d_return[0];
            sr_river_f[i][j][k][1]=sr_d_return[1];
          }
  ...
  for (k=1;k<60;k++)
  {
    ...
    {
      x2=0;
      y2=sr_river_f[0][j][k][1];

      x3=w;
      y3=y2;

      sr_d_cache_clr();
      l=sr_draw_line(x2,y2,x3,y3);
      if (l!=0) return(1);

      for (i=1;i<151;i++)
      {
        x4=sr_river_f[i][j][0][0];
        y4=0;

        x5=sr_river_f[i][j][0][0];
        y5=h-1;

        l=sr_draw_line2(x4,y4,x5,y5);
        if (l!=0) return(1);
...
static int  sr_river_show(VideoState *cur_stream)
{
  ...
  if (sr_river_f_init==0)
  {
    i=sr_river_f_cons();
    if (i!=0)
    {
      sr_river_f_init_fail=1;
      return(0);
    }
#if DPZ_DEBUG1
  ...
  sr_time1=tv.tv_sec;
  sr_time2=tv.tv_usec;
  sr_time5=sr_time1*1000000+sr_time2;

  li=sr_time5-sr_time3;	 // how many time have past , how many bytes played , how many left
  lj=2*sr_ch*44100*((float)li/(float)1000000/*-0.3*/);// in bytes
  lk=sr_river_adj+sr_total_bytes-lj;  //bytes

  if (lk<0)				   //bytes , for pause
  {
#if DPZ_DEBUG1
    sprintf(m604_str1,"show lk error1  lk=%lld, adj=%lld,bytes=%lld,now=%lld,not return",lk,sr_river_adj,sr_total_bytes,lj);
    deb_record(m604_str1);
#endif
    sr_river_adj=sr_river_adj-lk;
    lk=0;
  }

  if (lk>2*sr_ch*44100)			   //bytes , more than 1s , -0.05s
  {
    sr_river_adj=sr_river_adj-sr_ch*4096;
    lk=lk-sr_ch*4096;
  }

  if (lk>sr_sample_size*2)               //bytes
  {
#if DPZ_DEBUG1
    sprintf(m604_str1,"show lk error2  lk=%lld, adj=%lld,bytes=%lld,now=%lld,return >>>",lk,sr_river_adj,sr_total_bytes,lj);
    deb_record(m604_str1);
#endif
    // error
    return(0);
  ...
  lk=lk+2*sr_ch*44100*(float)0.1;     // bytes    delay 0.1s
  ...
  if (k<=cur_stream->sample_array_index)	// sample position of now
  {
    ...
    l=SR_FFT_BUFFER_SIZE*sr_ch*(l/(SR_FFT_BUFFER_SIZE*sr_ch));
  }
  else
  {
    if (sr_sample_over==1)
    {
      l=sr_sample_size+cur_stream->sample_array_index-k; // ring buffer
      l=SR_FFT_BUFFER_SIZE*sr_ch*(l/(SR_FFT_BUFFER_SIZE*sr_ch));
    }
    else
    {
  ...
  i=0;
  k=0;
  for (j=sr_river_ptr-1;j>=0;j--)
  {
    if (sr_river_mark[j]==l)
    {
      k=1;
      break;
    }
  ...
  if ((k==0)&&(i<396)&&(sr_river_over==1))
  {
    for (j=396-1;j>sr_river_ptr;j--)
    {
      if (sr_river_mark[j]==l)
      {
        k=1;
  ...
  m=j-6; //sr2
  if (m<0) m=396+m;
  if ((m/6)!=(sr_river_last/6))  //sr2 // error,not continue;return
  {
#if DPZ_DEBUG1
    sprintf(m604_str1,"show m=j-6=%d,j=%d, !=last_displayed ,    return >>>",m,j);
    deb_record(m604_str1);
#endif
    sr_river_last=(j/6)*6;  //sr2
    return(0);
  }
  ...
  for (n=0;n<150;n++)
    for (p=0;p<SR_FFT_BUFFER_SIZE/2;p++) sr_river2[n][p]=0;

  i=0;
  for (k=j;k>=0;k--)
  {
    for (n=0;n<SR_FFT_BUFFER_SIZE/2;n++) sr_river2[i][n]=sr_river[k][n];

    i++;
    if (i>=150) break;
  }

  if ((i<150)&&(sr_river_over==1))
  {
    for (k=396-1;k>sr_river_ptr;k--)
    {
      for (n=0;n<SR_FFT_BUFFER_SIZE/2;n++) sr_river2[i][n]=sr_river[k][n];
  ...
  freq_x=sr_river_f[149][2][0][0]+10;
  freq_y=sr_river_f[149][2][0][1];
  fill_rect_green(freq_x,freq_y,15,1,255);
  strcpy(str2,"300Hz");
  u_echo_str4screenstring(freq_x+20,freq_y-6,str2,300,2,0);

  //SDL_SetRenderDrawColor(renderer, 0, 255, 0, 0);

  freq_x=sr_river_f[149][6][0][0]+10;
  freq_y=sr_river_f[149][6][0][1];
  fill_rect_green(freq_x,freq_y,15,1,255);
  strcpy(str2,"1KHz");
  u_echo_str4screenstring(freq_x+20,freq_y-6,str2,300,2,0);
  ...
  freq_x=sr_river_f[149][12][0][0]+10;
  freq_y=sr_river_f[149][12][0][1];
  fill_rect_green(freq_x,freq_y,15,1,255);
  strcpy(str2,"2KHz");
  u_echo_str4screenstring(freq_x+20,freq_y-6,str2,300,2,0);
  ...
  freq_x=sr_river_f[149][29][0][0]+10;
  freq_y=sr_river_f[149][29][0][1];
  fill_rect_green(freq_x,freq_y,15,1,255);
  strcpy(str2,"5KHz");
  u_echo_str4screenstring(freq_x+20,freq_y-6,str2,300,2,0);
  ...
  freq_x=sr_river_f[149][59][0][0]+10;
  freq_y=sr_river_f[149][59][0][1];
  fill_rect_green(freq_x,freq_y,15,1,255);
  strcpy(str2,"10KHz");
  u_echo_str4screenstring(freq_x+20,freq_y-6,str2,300,2,0);

  // display comment
  strcpy(str2,"Every Column Has 256 Samples(44.1KHz)(5.80ms)");
  u_echo_str4screenstring(10,screen_h -18*3 -9-14-10,str2,300,2,0);
  ...
  for (i=0;i<=75-1;i++) // left half
  {
    for (k=0;k<118-1;k++)
    {
      ...
      if (n>0)
      {
	if (sr_river_f[i+1][k+1][0][1]-sr_river_f[i+1][k+1][n][1]>0)
	{
          //SDL_SetRenderDrawColor(renderer, s_p1, s_p2, s_p3, 0);
          //bgcolor = SDL_MapRGB(screen->format,s_p1,s_p2,s_p3);
          
	  fill_rect_green(sr_river_f[i  ][k+1][n][0],
			 sr_river_f[i  ][k+1][n][1],
			 sr_river_f[i+1][k+1][0][0]-sr_river_f[i  ][k+1][0][0], 
			 sr_river_f[i+1][k+1][0][1]-sr_river_f[i+1][k+1][n][1],s_p2);

          //SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0);
          //bgcolor2 = SDL_MapRGB(screen->format, 0, 0, 0);
          
	  fill_rect_green(sr_river_f[i  ][k+1][n][0],
			 sr_river_f[i  ][k+1][n][1],1,
			 sr_river_f[i  ][k+1][0][1]-sr_river_f[i  ][k+1][n][1],0);
	  fill_rect_green(sr_river_f[i+1][k+1][n][0],
			 sr_river_f[i+1][k+1][n][1],1,
			 sr_river_f[i+1][k+1][0][1]-sr_river_f[i+1][k+1][n][1],0);
	}
/*
	sr_draw_line3(sr_river_f[i  ][k+1][0][0],
				  sr_river_f[i  ][k+1][0][1],
				  sr_river_f[i+1][k+1][0][0],
				  sr_river_f[i+1][k+1][0][1]);
	sr_draw_line3(sr_river_f[i  ][k+1][n][0],
				  sr_river_f[i  ][k+1][n][1],
				  sr_river_f[i+1][k+1][n][0],
				  sr_river_f[i+1][k+1][n][1]);
*/
        //SDL_SetRenderDrawColor(renderer, s_p1, s_p2, s_p3, 0);
        //bgcolor = SDL_MapRGB(screen->format,s_p1,s_p2,s_p3);
        
	fill_rect_green(sr_river_f[i  ][k+1][0][0],
		       sr_river_f[i  ][k+1][0][1],
		       sr_river_f[i+1][k+1][0][0]-sr_river_f[i  ][k+1][0][0], 
		       1,s_p2);
	...
	fill_rect_green(sr_river_f[i  ][k+1][n][0],
		       sr_river_f[i  ][k+1][n][1],
		       sr_river_f[i+1][k+1][n][0]-sr_river_f[i  ][k+1][n][0], 
		       1,s_p2);
	...
	sr_draw_line4(sr_river_f[i+1][k+1][n][0],
			  sr_river_f[i+1][k+1][n][1],
			  sr_river_f[i+1][k  ][n][0],
			  sr_river_f[i+1][k  ][n][1],0);

	sr_draw_line4(sr_river_f[i+1][k+1][0][0],
			  sr_river_f[i+1][k+1][0][1],
			  sr_river_f[i+1][k  ][0][0],
			  sr_river_f[i+1][k  ][0][1],1);

	s_first=1;

	while(1)
	{
	  s_x1=sr_draw_line4_get(0);
	  if (s_x1<0) break;

	  s_x2=sr_draw_line4_get(1);
	  if (s_x2<0) break;
	  ...
	  if (sr_d_line_dot[1][1]-sr_d_line_dot[0][1]>0)
	  {
            //SDL_SetRenderDrawColor(renderer, s_p4, s_p5, s_p6, 0);
            //bgcolor3 = SDL_MapRGB(screen->format,s_p4,s_p5,s_p6);

	    fill_rect_green(sr_d_line_dot[0][0],
			   sr_d_line_dot[0][1],1,
			   sr_d_line_dot[1][1]-sr_d_line_dot[0][1],s_p5);
	  }
	}

	if (sr_river_f[i+1][k  ][0][1]-sr_river_f[i+1][k  ][n][1]>0)
	{
          //SDL_SetRenderDrawColor(renderer, s_p4, s_p5, s_p6, 0);
          //bgcolor3 = SDL_MapRGB(screen->format,s_p4,s_p5,s_p6);
          
          // note ,for smaller moniter,bgcolor2 change to bgcolor3
	  fill_rect_green(sr_river_f[i+1][k  ][n][0],
			 sr_river_f[i+1][k  ][n][1],1,
			 sr_river_f[i+1][k  ][0][1]-sr_river_f[i+1][k  ][n][1],s_p5);
	}

	sr_draw_line3(sr_river_f[i+1][k+1][n][0],
			  sr_river_f[i+1][k+1][n][1],
			  sr_river_f[i+1][k  ][n][0],
			  sr_river_f[i+1][k  ][n][1]);

	sr_draw_line3(sr_river_f[i+1][k+1][0][0],
			  sr_river_f[i+1][k+1][0][1],
			  sr_river_f[i+1][k  ][0][0],
			  sr_river_f[i+1][k  ][0][1]);
      }
      ...
      sr_draw_line4(sr_river_f[i  ][k+1][n][0],
			sr_river_f[i  ][k+1][n][1],
			sr_river_f[i  ][k  ][n][0],
			sr_river_f[i  ][k  ][n][1],0);
      ...
      while(1)
      {
	s_y1=sr_draw_line4_get2(0);
	if (s_y1<0) break;

	s_y2=sr_draw_line4_get2(1);
	if (s_y2<0) break;
	...
	fill_rect_green(sr_d_line_dot[0][0],
		       sr_d_line_dot[0][1],
		       sr_d_line_dot[1][0]-sr_d_line_dot[0][0],
		       1,s_p8);
      }
      ...
      fill_rect_green(sr_river_f[i  ][k  ][n][0],
		     sr_river_f[i  ][k  ][n][1],
		     sr_river_f[i+1][k  ][n][0]-sr_river_f[i  ][k  ][n][0],
		     1,0);
// / * repeated
      fill_rect_green(sr_river_f[i  ][k+1][n][0],
		     sr_river_f[i  ][k+1][n][1],
...
*/
	...
	sr_draw_line4(sr_river_f[i  ][k  ][n][0],
			  sr_river_f[i  ][k  ][n][1],
			  sr_river_f[i  ][k+1][n][0],
			  sr_river_f[i  ][k+1][n][1],0);

	sr_draw_line4(sr_river_f[i  ][k  ][0][0],
			  sr_river_f[i  ][k  ][0][1],
			  sr_river_f[i  ][k+1][0][0],
			  sr_river_f[i  ][k+1][0][1],1);
	...
	while(1)
	{
	  ...
	  if (sr_d_line_dot[1][1]-sr_d_line_dot[0][1]>0)
	  {
	    ...
	    fill_rect_green(sr_d_line_dot[0][0],
			   sr_d_line_dot[0][1],1,
			   sr_d_line_dot[1][1]-sr_d_line_dot[0][1],s_p11);
	  }
	}

	if (sr_river_f[i  ][k  ][0][1]-sr_river_f[i  ][k  ][n][1]>0)
	{
          //SDL_SetRenderDrawColor(renderer, s_p10, s_p11, s_p12, 0);
          //bgcolor5 = SDL_MapRGB(screen->format,s_p10,s_p11,s_p12);
	  ...
	  fill_rect_green(sr_river_f[i  ][k  ][n][0],
			 sr_river_f[i  ][k  ][n][1],1,
			 sr_river_f[i  ][k  ][0][1]-sr_river_f[i  ][k  ][n][1],s_p11);
	}

	sr_draw_line3(sr_river_f[i  ][k  ][n][0],
			  sr_river_f[i  ][k  ][n][1],
			  sr_river_f[i  ][k+1][n][0],
			  sr_river_f[i  ][k+1][n][1]);

	sr_draw_line3(sr_river_f[i  ][k  ][0][0],
			  sr_river_f[i  ][k  ][0][1],
			  sr_river_f[i  ][k+1][0][0],
			  sr_river_f[i  ][k+1][0][1]);
      }
      ...
      sr_draw_line4(sr_river_f[i+1][k  ][n][0],
			sr_river_f[i+1][k  ][n][1],
			sr_river_f[i+1][k+1][n][0],
			sr_river_f[i+1][k+1][n][1],1);
...
// / *  repeated
      fill_rect_green(sr_river_f[i  ][k+1][n][0],
		     ...
		     1,0);
// * /
      sr_draw_line3(sr_river_f[i+1][k  ][n][0],
			sr_river_f[i+1][k  ][n][1],
			sr_river_f[i+1][k+1][n][0],
			sr_river_f[i+1][k+1][n][1]);
...
static int  sr_river_f_cons_test(VideoState *cur_stream,int ptr)
{
  ...
  {
    i=sr_river_f_cons();
    if (i!=0) sr_river_f_init_fail=1;
  }

  SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0);
  //bgcolor = SDL_MapRGB(screen->format, 0x00, 0x00, 0x00);

  fill_rectangle(0 ,0 , cur_stream->width , cur_stream->height -/*deb_ch_h*/ 18*3 -9 ); 
  ...
  for (i=0;i<151;i++)
  {
	...
	if (k>=60) continue;
        ...
        x1=sr_river_f[i][j][k][0];
        y1=sr_river_f[i][j][k][1];
  ...
  // display comment
  strcpy(str1,"Every Column Has 256 Samples(44.1KHz)(5.80ms)");
  u_echo_str4screenstring(10,screen_h -18*3 -9 -14-10,str1,300,0,1);
...
static int sr_draw_line(int x1,int y1,int x2,int y2)
{
  ...
  sr_d_cache[sr_d_cache_ptr][0]=x1;
  sr_d_cache[sr_d_cache_ptr][1]=y1;
  sr_d_cache_ptr++;
  ...
  sr_d_cache[sr_d_cache_ptr][0]=x2;
  sr_d_cache[sr_d_cache_ptr][1]=y2;
  sr_d_cache_ptr++;
  ...
  if (x1==x2)
  {
    if (y1<y2)
    {
      for (i=y1+1;i<=y2-1;i++)// sr_d_buff[x1][i]=1;
      {
	if ((x1<0)||(x1>=7680)) return(0);
	...
	sr_d_buff[x1][i]=1;
        ...
        sr_d_cache[sr_d_cache_ptr][0]=x1;
        sr_d_cache[sr_d_cache_ptr][1]=i;
        sr_d_cache_ptr++;
      }
    }
    else if (y2<y1)
    {
      for (i=y1-1/*y2+1*/;i>=y2+1/*y1-1*/;i--) //sr_d_buff[x1][i]=1;
      {
	if ((x1<0)||(x1>=7680)) return(0);
	if ((i<0)||(i>=4320)) return(0);
	sr_d_buff[x1][i]=1;
  ...
  else
  {
    if (y1==y2)
    {
      for (i=x1+1;i<=x2-1;i++)// sr_d_buff[i][y1]=1;
      {
	if ((i<0)||(i>=7680)) return(0);
	if ((y1<0)||(y1>=4320)) return(0);
	sr_d_buff[i][y1]=1;

        sr_d_cache[sr_d_cache_ptr][0]=i;
        sr_d_cache[sr_d_cache_ptr][1]=y1;
        sr_d_cache_ptr++;
      }
    }
	...
	if ((y4<0)||(y4>=4320)) return(0);
        ...
        sr_d_cache[sr_d_cache_ptr][0]=x4;
        sr_d_cache[sr_d_cache_ptr][1]=y4;
        sr_d_cache_ptr++;

        if (y4>y5+1)
        {
          for (j=y5+1;j<y4;j++)// sr_d_buff[x5][j]=1;
	  {
	    if ((x5<0)||(x5>=7680)) return(0);
	    if ((j<0)||(j>=4320)) return(0);
	    sr_d_buff[x5][j]=1;

            sr_d_cache[sr_d_cache_ptr][0]=x5;
            sr_d_cache[sr_d_cache_ptr][1]=j;
            sr_d_cache_ptr++;
	  }
        }
...
static int sr_draw_line3(int x1,int y1,int x2,int y2)
{
  ...
  if (x1==x2)
  {
	...
	if ((x1<0)||(x1>=7680)) return(1);
	if ((i<0)||(i>=4320)) return(1);
	fill_rect_green(x1,i,1,1,0);//sr_d_buff[x1][i]=1;
      }
    }
    else if (y2<y1)
    {
  ...
  else
  {
	...
	if ((i<0)||(i>=7680)) return(1);
	if ((y1<0)||(y1>=4320)) return(1);
	fill_rect_green(i,y1,1,1,0);//sr_d_buff[i][y1]=1;
      }
    }
    else if (y2>y1)
    {
	...
	if ((x4<0)||(x4>=7680)) return(1);
	if ((y4<0)||(y4>=4320)) return(1);
        fill_rect_green(x4,y4,1,1,0);//sr_d_buff[x4][y4]=1;
          ...
          for (j=y5+1;j<y4;j++)
	  {
	    if ((x5<0)||(x5>=7680)) return(1);
	    if ((j<0)||(j>=4320)) return(1);
	    fill_rect_green(x5,j,1,1,0);//sr_d_buff[x5][j]=1;
	  }
        }
...
static int sr_draw_line4(int x1,int y1,int x2,int y2,int ptr)
{
  ...
  if ((x1<0)||(x1>=7680)) return(0);
  if ((y1<0)||(y1>=4320)) return(0);
  sr_d_line[ptr][ptr2][0]=x1;
  sr_d_line[ptr][ptr2][1]=y1;
  ptr2++;
  ...
  if (x1==x2)
  {
    if (y1<y2)
    {
      for (i=y1+1;i<=y2-1;i++) //fill_rectangle(screen,x1,i,1,1,bgcolor,0);//sr_d_buff[x1][i]=1;
      {
	  if ((ptr <0)||(ptr >=2)) return(0);
	  if ((ptr2<0)||(ptr2>=1000)) return(0);
	  if ((x1<0)||(x1>=7680)) return(0);
	  if ((i<0)||(i>=4320)) return(0);
	  sr_d_line[ptr][ptr2][0]=x1;
	  sr_d_line[ptr][ptr2][1]=i;
	  ptr2++;
      }
    }
    else if (y2<y1)
    {
      for (i=y1-1/*y2+1*/;i>=y2+1/*y1-1*/;i--) //fill_rectangle(screen,x1,i,1,1,bgcolor,0); //sr_d_buff[x1][i]=1;
      {
	if ((ptr <0)||(ptr >=2)) return(0);
	if ((ptr2<0)||(ptr2>=1000)) return(0);
	if ((x1<0)||(x1>=7680)) return(0);
  ...
  else
  {
    if (y1==y2)
    {
      for (i=x1+1;i<=x2-1;i++) //fill_rectangle(screen,i,y1,1,1,bgcolor,0);//sr_d_buff[i][y1]=1;
      {
	  if ((ptr <0)||(ptr >=2)) return(0);
	  if ((ptr2<0)||(ptr2>=1000)) return(0);
	  if ((i<0)||(i>=7680)) return(0);
	  if ((y1<0)||(y1>=4320)) return(0);
	  sr_d_line[ptr][ptr2][0]=i;
	  sr_d_line[ptr][ptr2][1]=y1;
	  ptr2++;
      }
    }
    else if (y2>y1)
    {
      ...
      for (i=1;i<=x2-x1;i++)
      {
        ...
        //fill_rectangle(screen,x4,y4,1,1,bgcolor,0);//sr_d_buff[x4][y4]=1;

        if (y4>y5+1)
        {
          for (j=y5+1;j<y4;j++) //fill_rectangle(screen,x5,j,1,1,bgcolor,0);//sr_d_buff[x5][j]=1;
	  {
	    if ((ptr <0)||(ptr >=2)) return(0);
	    if ((ptr2<0)||(ptr2>=1000)) return(0);
	    if ((x5<0)||(x5>=7680)) return(0);
	    if ((j<0)||(j>=4320)) return(0);
	    sr_d_line[ptr][ptr2][0]=x5;
	    sr_d_line[ptr][ptr2][1]=j;
	    ptr2++;
	  }
        }
	...
	if ((x4<0)||(x4>=7680)) return(0);
	if ((y4<0)||(y4>=4320)) return(0);
	sr_d_line[ptr][ptr2][0]=x4;
	sr_d_line[ptr][ptr2][1]=y4;
	ptr2++;
  ...
  if ((x2<0)||(x2>=7680)) return(0);
  if ((y2<0)||(y2>=4320)) return(0);
  sr_d_line[ptr][ptr2][0]=x2;
  sr_d_line[ptr][ptr2][1]=y2;
  ptr2++;

  sr_d_line_ptr[ptr]=ptr2;
...
static int sr_draw_line4_get(int ptr)
{
	...
	i=sr_d_line_ptr2[ptr];

	while(1)
	{
	        if (i<0) break;
		if (i>=1000) break;
                if (i>=sr_d_line_ptr[ptr]) break;

		x1=sr_d_line[ptr][i][0];
		y1=sr_d_line[ptr][i][1];

		if (x1<0) return(x1);
  
		if (x1==sr_d_line_dot[ptr][0])
		{
			i++;
			continue;
		}
		
		sr_d_line_dot[ptr][0]=x1;
		sr_d_line_dot[ptr][1]=y1;
		i++;
		sr_d_line_ptr2[ptr]=i;
		return(x1);
	}
...
int clr_rect_black(void)
{
	...
	j=stream_open_is->width * (stream_open_is->height-18*3-9);
	for (i=0;i<k;i++)
	{
	  if (i<j)
	  {
        ...
        g_draw_line(0,stream_open_is->height-18*3-9,stream_open_is->width,stream_open_is->height-18*3-9,0,128,128,128);
...
static int pl_no_support(char *p_str,int p_str_size)
{
  ...
  if (i!=0) return(1);
  if (strlen(m302_str1)>7) return(1);
  ...
  if (strcmp(m302_str1,"rpm")==0) return(1);
  if (strcmp(m302_str1,"iso")==0) return(1);
  if (strcmp(m302_str1,"7z")==0) return(1);
  
  if (strcmp(m302_str1,"txt")==0) return(1);
  if (strcmp(m302_str1,"texi")==0) return(1);
  ...
  if (strcmp(m302_str1,"webp")==0) return(1);
  ...
  if (strcmp(m302_str1,"shtml")==0) return(1);
  if (strcmp(m302_str1,"xml")==0) return(1);
  if (strcmp(m302_str1,"asp")==0) return(1);
  if (strcmp(m302_str1,"aspx")==0) return(1);
  ...
  if (strcmp(m302_str1,"asm")==0) return(1);

  if (strcmp(m302_str1,"java")==0) return(1);

  if (strcmp(m302_str1,"py")==0) return(1);

  if (strcmp(m302_str1,"torrent")==0) return(1);

  if (strcmp(m302_str1,"sc0")==0) return(1);
  if (strcmp(m302_str1,"sc2")==0) return(1);
  if (strcmp(m302_str1,"sc3")==0) return(1);
  if (strcmp(m302_str1,"sc4")==0) return(1);
  if (strcmp(m302_str1,"sc5")==0) return(1);
  if (strcmp(m302_str1,"sc6")==0) return(1);
  if (strcmp(m302_str1,"tb0")==0) return(1);
  if (strcmp(m302_str1,"tb1")==0) return(1);
  if (strcmp(m302_str1,"tb2")==0) return(1);
  if (strcmp(m302_str1,"tb3")==0) return(1);
  if (strcmp(m302_str1,"tb5")==0) return(1);
  if (strcmp(m302_str1,"tb6")==0) return(1);
  if (strcmp(m302_str1,"dbf")==0) return(1);
...
int g_paint_ini(void)
{
  ...
  if (realloc_texture(&stream_open_is->vis_texture, SDL_PIXELFORMAT_ARGB8888, screen_w, screen_h, SDL_BLENDMODE_NONE, 1) < 0)
            return(0);
  ...
  if (!SDL_LockTexture(stream_open_is->vis_texture, &deb_tx_rect, (void **)&deb_tx_pixels, &deb_tx_pitch)) 
  {
                deb_tx_pitch >>= 2;
  }
  else return(0);
...
int g_cut_init(void)
{
  ...
  strcpy(g_cut_map2[2][0]," *");
  strcpy(g_cut_map2[2][1],"*.");
  
  strcpy(g_cut_map2[3][0]," **");
  strcpy(g_cut_map2[3][1],"*..");
  strcpy(g_cut_map2[3][2],"*..");
  
  strcpy(g_cut_map2[4][0],"  **");
  strcpy(g_cut_map2[4][1]," *..");
  strcpy(g_cut_map2[4][2],"*...");
  strcpy(g_cut_map2[4][3],"*...");
  
  strcpy(g_cut_map2[5][0],"  ***");
  strcpy(g_cut_map2[5][1]," *...");
  strcpy(g_cut_map2[5][2],"*....");
  strcpy(g_cut_map2[5][3],"*....");
  strcpy(g_cut_map2[5][4],"*....");
  
  strcpy(g_cut_map2[6][0],"   ***");
  strcpy(g_cut_map2[6][1]," *....");
  strcpy(g_cut_map2[6][2]," *....");
  strcpy(g_cut_map2[6][3],"*.....");
  strcpy(g_cut_map2[6][4],"*.....");
  strcpy(g_cut_map2[6][5],"*.....");
...
{
  int			i,j,j5,k,l,m,m2,n,o,p,q,r,t;
  int			x,y;
  ...
  for (i=0;i<g_button_ptr;i++)
  {
    ...
    for (n=0;n<l;n++)
    {
      for (o=0;o<m;o++)
      {
        ...
        if (g_button_enable[i]==1)
        {
          ...
          {
            if ((g_mouse_down==1)&&(g_detect_ptr1==3)&&(g_detect_ptr2==i))
            {
              if (vid)
              {
                  SDL_SetRenderDrawColor(renderer, i1*7/10, i2*7/10, i3*7/10, i4);

                  deb_fill_rectangle(j+n, k+o, 1, 1);
              }
              else
              {
    ...
    while(j5<len)
    {
      if (strlen(str)>j5+0) c1=(unsigned char)str[j5+0];
      else c1=0;
    
      if (strlen(str)>j5+1) c2=(unsigned char)str[j5+1];
      else c2=0;
    
      if (strlen(str)>j5+2) c3=(unsigned char)str[j5+2];
      else c3=0;
      ...
      if ((u_err==0)&&(u_err2==0))
      {
		  ...
		  if (g_button_black[i]==0)
		  {
                ...
                if (vid)
                {
                  ...
                  deb_fill_rectangle(x+m2, y+n, 1, 1);
                }
                else
                {
  ...
  for (i=0;i<g_scrollbar_ptr;i++)
  {
    ...
    if (hori==0)
    {
	  ...
	  if      ((n==0)||(n==l-1)) { j1=128;j2=128;j3=128; }
          ...
          if (g_scrollbar_enable[i]==1)
          {
            if (vid)
            {
                  ...
                  deb_fill_rectangle(j+n, q+o, 1, 1);
            }
            else
            {
	  ...
	  if      ((n==0)||(n==4  )) { j1=128;j2=128;j3=128; }
          ...
          if (g_scrollbar_enable[i]==1)
          {
            ...
            {
                  SDL_SetRenderDrawColor(renderer, j1, j2, j3, i4);
                  deb_fill_rectangle(r+n, t+o, 1, 1);
            }
            else
            {
    ...
    else if (hori==1)
    {
      ...
      for (n=m-1;n>=0;n--)
      {
        ...
        {
          if ((n==0  )&&(o==0)) continue;
          if ((n==m-1)&&(o==0)) continue;
          if ((n==m-1)&&(o==4)) continue;
          if ((n==0  )&&(o==4)) continue;

	  if      ((n==0)||(n==m-1)) { j1=128;j2=128;j3=128; }
	  else if ((o==0)||(o==4  )) { j1=128;j2=128;j3=128; }
	  else    { j1=i1;j2=i2;j3=i3; }
          ...
          if (g_scrollbar_enable[i]==1)
          {
            ...
            {
                  SDL_SetRenderDrawColor(renderer, j1, j2, j3, i4);
                  deb_fill_rectangle(q+o,k+n,/* q+o,*/ 1, 1);
            }
            else
            {
                  deb_set_dot(q+o,k+n,/*q+o,*/j1,j2,j3);
            }
          }
          else
          {
            if (g_scrollbar_black[i]==0)
            {
              ...
              {
                  deb_set_dot(q+o,k+n,/*q+o,*/j1*7/10,j2*7/10,j3*7/10);
              }
            }
            else
            {
              ...
              {
                  deb_set_dot(q+o,k+n,/*q+o,*/255-(255-j1)*7/10,255-(255-j2)*7/10,255-(255-j3)*7/10);
              }
            }
          }
        }
	  ...
	  if      ((n==0)||(n==4  )) { j1=128;j2=128;j3=128; }
          ...
          if (g_scrollbar_enable[i]==1)
          {
            ...
            {
                  SDL_SetRenderDrawColor(renderer, j1, j2, j3, i4);
                  deb_fill_rectangle(t+o,r+n,/* t+o,*/ 1, 1);
            }
            else
            {
                  deb_set_dot(t+o,r+n,/*t+o,*/j1,j2,j3);
            }
          }
          else
          {
            if (g_scrollbar_black[i]==0)
            {
              ...
              {
                  deb_set_dot(t+o,r+n,/*t+o,*/j1*7/10,j2*7/10,j3*7/10);
              }
            }
            else
            {
              ...
              {
                  deb_set_dot(t+o,r+n,/*t+o,*/255-(255-j1)*7/10,255-(255-j2)*7/10,255-(255-j3)*7/10);
              }
            }
          }
        }
...
int g_paint_dirview(void)
{
  ...
  for (i=0;i<g_dirview_ptr;i++)
  {
    ...
    //ftree_filenamebuff_ptr
    //ftree_filenamebuff_n+1
    //ftree_filenamebuff_n+m/14
      
    if (ftree_filenamebuff_ptr<=m/(14+2))
    {
      g_dirview_top=0;
      continue;
    }
      
    r=(m-2)*((float)ftree_filenamebuff_n          )/(float)ftree_filenamebuff_ptr;
    s=(m-2)*((float)ftree_filenamebuff_n+m/(14+2) )/(float)ftree_filenamebuff_ptr;
...
/*
  stream_open_is = malloc(sizeof(VideoState));
  if (!stream_open_is) return(1);
*/
  deb_ini_is(stream_open_is);
...
int g_detect_click(int x,int y)
{
  ...
  for (i=7;i<=7;i++)
  {
    if (i==7)
    {
      for (j=g_menu_paint_ptr-1;j>=0;j--)
      {
        if ((g_menu_paint_x[j]<=x)&&(g_menu_paint_y[j]<=y)&&(x<g_menu_paint_x[j]+g_menu_paint_w[j])&&(y<g_menu_paint_y[j]+g_menu_paint_h[j]))
        {
          g_detect_ptr1=i;
          g_detect_ptr2=j;
	  g_detect_ptr3=(y-g_menu_paint_y[j]-8)/16;
	  g_detect_ptr4=g_menu_paint_menu[j];
    ...
    if (i==1)
    {
      for (j=0;j<g_dirview_ptr;j++)
      {
        if ((g_dirview_posi[j][0]<=x)&&(g_dirview_posi[j][1]<=y)&&(x<g_dirview_posi[j][0]+g_dirview_posi[j][2])&&(y<g_dirview_posi[j][1]+g_dirview_posi[j][3]))
        {
          ...
          {
            g_detect_ptr3=1;
            g_detect_ptr4=(y-g_dirview_posi[j][1])/(14+2);
          }
    ...
    if (i==2)
    {
      for (j=0;j<g_scrollbar_ptr;j++)
      {
        if ((g_scrollbar_posi[j][0]<=x)&&(g_scrollbar_posi[j][1]<=y)&&(x<g_scrollbar_posi[j][0]+g_scrollbar_posi[j][2])&&(y<g_scrollbar_posi[j][1]+g_scrollbar_posi[j][3]))
        {
          ...
          if (g_scrollbar_hori[j]==0)
          {
              g_detect_ptr3=x-g_scrollbar_posi[j][0];
              g_detect_ptr4=g_scrollbar_posi[j][2];
          }
          else
          {
              g_detect_ptr3=g_scrollbar_posi[j][1]+g_scrollbar_posi[j][3]-y;
              g_detect_ptr4=g_scrollbar_posi[j][3];
          }
...
int g_detect_motion(int x,int y)
{
  ...
  for (i=7;i<=7;i++)
  {
    if (i==7)
    {
	  ...
	  g_detect_motion_ptr3=(y-g_menu_paint_y[j]-8)/16;
	  g_detect_motion_ptr4=g_menu_paint_menu[j];
...
int g_icon_id(int dir,char *p_str,int p_str_size)
{
  ...
  ftree_filename_ext(p_str,p_str_size,m301_str1,300);
  ...
  for (i=0;i<g_lineedit_ptr;i++)
  {
    ...
    for (n=0;n<l;n++)
    {
      if (vid)
      {
                  ...
                  deb_fill_rectangle(j+n, k, 1, 1);
                  deb_fill_rectangle(j+n, k+m,1, 1);
      }
      else
      {
    ...
    for (o=0;o<m;o++)
    {
      ...
      {
                  SDL_SetRenderDrawColor(renderer, k1, k2, k3, k4);
                  deb_fill_rectangle(j, k+o, 1, 1);
                  deb_fill_rectangle(j+l,k+o, 1, 1);
      }
      else
      {
    ...
    if ((g_focus_ptr1==4)&&(g_focus_ptr2==i)&&(g_caret_show==1))
    {
      ...
      for (o=0;o<14;o++)
      {
        ...
        {
                  SDL_SetRenderDrawColor(renderer, i1, i2, i3, i4);
                  deb_fill_rectangle(j+2+r*7, k+1+o, 1, 1);
  ...
  for (i=0;i<g_checkbox_ptr;i++)
  {
    ...
    for (n=0;n<l;n++)
    {
      for (o=0;o<m;o++)
      {
        if (g_checkbox_enable[i]==1)
        {
          ...
          {
            if ((g_mouse_down==1)&&(g_detect_ptr1==5)&&(g_detect_ptr2==i))
            {
              if (vid)
              {
                  SDL_SetRenderDrawColor(renderer, i1*7/10, i2*7/10, i3*7/10, i4);
    ...
    for (n=1;n<l-1-2;n++)
    {
      if ((g_mouse_down==1)&&(g_detect_ptr1==5)&&(g_detect_ptr2==i))
      {
        if (vid)
        {
                  ...
                  deb_fill_rectangle(j+n,k+1+2  , 1, 1);
                  deb_fill_rectangle(j+n,k+m-1-1, 1, 1);
        }
        else
        {
    ...
    for (o=1+2;o<m-1;o++)
    {
      if ((g_mouse_down==1)&&(g_detect_ptr1==5)&&(g_detect_ptr2==i))
      {
        ...
        {
                  SDL_SetRenderDrawColor(renderer, k1*7/10, k2*7/10, k3*7/10, k4);
                  deb_fill_rectangle(j+1      ,k+o, 1, 1);
                  deb_fill_rectangle(j+l-1-1-2,k+o, 1, 1);
        }
        else
        {
...
int g_paint_radiobutton(int vid)
{
  int			i,j,k,l,m;
  int  			i1,i2,i3;
  int                   k1,k2,k3;
  ...
  for (i=0;i<g_radiobutton_ptr;i++)
  {
    ...
    if (g_radiobutton_enable[i]==1)
    {
      ...
      {
        if ((g_mouse_down==1)&&(g_detect_ptr1==6)&&(g_detect_ptr2==i))
        {
...
int g_def_proc(SDL_Event event,int vid)
{
    ...
    switch (event.type) 
    {
        ...
        case SDL_KEYUP:
            ...
            if (!( mod & KMOD_CAPS ))  g_key_caps =0;
            if (!( mod & KMOD_SHIFT))  g_key_shift=0;
            if (!( mod & KMOD_CTRL ))  g_key_ctrl =0;

	    printf("key up:caps=%d,shift=%d,ctrl=%d,\n\n",g_key_caps,g_key_shift,g_key_ctrl);
        ...
        case SDL_KEYDOWN:
            ...
            if ( mod & KMOD_CAPS )  g_key_caps =1;
            if ( mod & KMOD_SHIFT)  g_key_shift=1;
            if ( mod & KMOD_CTRL )  g_key_ctrl =1;

	    printf("key down:caps=%d,shift=%d,ctrl=%d,\n\n",g_key_caps,g_key_shift,g_key_ctrl);
		...
		break;
            ...
            case SDLK_a:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='A';
                else smg_key='a';
                break;
            case SDLK_b:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='B';
                else smg_key='b';
                break;
            case SDLK_c:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='C';
                else smg_key='c';
                break;
            case SDLK_d:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='D';
                else smg_key='d';
                break;
            case SDLK_e:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='E';
                else smg_key='e';
                break;
            case SDLK_f:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='F';
                else smg_key='f';
                break;
            case SDLK_g:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='G';
                else smg_key='g';
                break;
            case SDLK_h:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='H';
                else smg_key='h';
                break;
            case SDLK_i:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='I';
                else smg_key='i';
                break;
            case SDLK_j:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='J';
                else smg_key='j';
                break;
            case SDLK_k:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='K';
                else smg_key='k';
                break;
            case SDLK_l:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='L';
                else smg_key='l';
                break;
            case SDLK_m:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='M';
                else smg_key='m';
                break;
            case SDLK_n:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='N';
                else smg_key='n';
                break;
            case SDLK_o:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='O';
                else smg_key='o';
                break;
            case SDLK_p:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='P';
                else smg_key='p';
                break;
            case SDLK_q:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='Q';
                else smg_key='q';
                break;
            case SDLK_r:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='R';
                else smg_key='r';
                break;
            case SDLK_s:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='S';
                else smg_key='s';
                break;
            case SDLK_t:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='T';
                else smg_key='t';
                break;
            case SDLK_u:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='U';
                else smg_key='u';
                break;
            case SDLK_v:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='V';
                else smg_key='v';
                break;
            case SDLK_w:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='W';
                else smg_key='w';
                break;
            case SDLK_x:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='X';
                else smg_key='x';
                break;
            case SDLK_y:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='Y';
                else smg_key='y';
                break;
            case SDLK_z:
                if ((g_key_caps==1)||(g_key_shift==1)) smg_key='Z';
                else smg_key='z';
                break;
	...
	case USER_DOUBLECLICK_EVENT:
	  ...
	  if (g_dirview_mouse_down==1)
	  {
            ...
            g_mouse_cntr++;
            g_mouse_cntr=(g_mouse_cntr % 3);
            if (g_mouse_cntr!=0) break;
	    ...
	    if (yy2>g_dirview_mouse_down_y)
	      ...
	      ftree_filenamebuff_n=ftree_filenamebuff_ptr*(g_dirview_bottom-g_dirview_posi[0][1]-1)/(g_dirview_posi[0][3]-2);
	      
	      if (ftree_filenamebuff_n>=g_dirview_posi[0][3]/(deb_ch_h+2))
	      {
	        ftree_filenamebuff_n=ftree_filenamebuff_n-g_dirview_posi[0][3]/(deb_ch_h+2)+1;
	      }
	      else
	      {
	        ftree_filenamebuff_n=0;
	      }
    	    }
	    else if (yy2<g_dirview_mouse_down_y)
	    {
	      ...
	      ftree_filenamebuff_n=ftree_filenamebuff_ptr*(g_dirview_top-g_dirview_posi[0][1]-1)/(g_dirview_posi[0][3]-2);
    	    }  	    
	    
	    //pl_disp_bar(stream_open_is);
...
/*
	    ...
	    if (yy2>g_dirview_mouse_down_y)
	      ...
	      if (ftree_filenamebuff_n>=g_dirview_posi[0][3]/deb_ch_h)
	      {
	        ftree_filenamebuff_n=ftree_filenamebuff_n-g_dirview_posi[0][3]/deb_ch_h+1;
	      }
	      else
	      {
	        ftree_filenamebuff_n=0;
...
int g_def_proc_paint(int vid)
{
    if (deb_m_ref==1)   // in SDL2,you need prepare full screen data,if up screen is painted,you need paint down screen,
    {                   // if up screen isn't paint,you can't paint down screen
      //g_paint_dirview();
      g_paint_scrollbar(vid);         // vid==1 means video screen,vid==0 means dirview screen
      g_paint_button(vid);
      g_paint_label(vid);
      g_paint_checkbox(vid);
      g_paint_radiobutton(vid);
      g_paint_lineedit(vid);
      
      g_paint_menu();   // last shown at front

      if (deb_tx_locked==1)
      {
        SDL_UnlockTexture(stream_open_is->vis_texture);
        SDL_RenderCopy(renderer, stream_open_is->vis_texture, NULL, NULL);
        deb_tx_locked=0;
      }
      SDL_RenderPresent(renderer);
      deb_m_ref=0;
...
int g_menu_load(int ptr)
{
  ...
  if ((ptr<0)||(ptr>=10)) return(0); // max 10 menus
  
  c1='0'+ptr;
  str2[0]=c1;
  str2[1]=0;
  
  strcpy(str1,"./utf8_bmp/menu");
  strcat(str1,str2);
  strcat(str1,".txt");
  ...
  if (fp1==NULL)
  {
    printf("Open file './utf8_bmp/menuN.txt' error\n");
    return(1);
  }
  ...
  while (!feof(fp1))
  {
    ...
    string_trim_nos(str3);
    strltrim(str3,300);
    
    if (strncmp(str3,"//",2)==0) continue;
    if ((str3[0]>=0)&&(str3[0]<' ')) continue;
    ...
    if (strlen(str4[1])<10 ) strcpy(g_menu_item_father[ptr][m],str4[1]);
    if (strlen(str4[3])<300) strcpy(g_menu_item_name[ptr][m],str4[3]);
    if (strlen(str4[5])<10 ) strcpy(g_menu_item_self[ptr][m],str4[5]);
    
    if (str4[6][0]=='i') g_menu_item_haschild[ptr][m]=1;
    if (str4[7][0]=='s') g_menu_item_separate[ptr][m]=1;
    
    m++;
  }
  
  g_menu_item_ptr[ptr] =m;
  g_menu_item_load[ptr]=1;
  ...
  for (i=0;i<g_menu_item_ptr[ptr];i++)
  {
    printf("%s  %s  %s  %d  %d  \n",
       g_menu_item_father[ptr][i],
       g_menu_item_name[ptr][i],
       g_menu_item_self[ptr][i],
       g_menu_item_haschild[ptr][i],
       g_menu_item_separate[ptr][i]);
  }
...
int g_menu_paint_prepare(int x,int y,int menuptr,char *father)
{
  ...
  if (g_menu_item_load[menuptr]!=1) return(1);
  
  n1=(-1);
  n3=0;
  
  for (i=0;i<g_menu_item_ptr[menuptr];i++)
  {
    if (strcmp(g_menu_item_father[menuptr][i],father)==0)
    {
      if (n1<0)  //first item
      {
        n1=i;
      ...
      if (u_strlen(g_menu_item_name[menuptr][i],300)>n3) n3=u_strlen(g_menu_item_name[menuptr][i],300);
    }
  }
  ...
  if ((x+n4<stream_open_is->width)&&(y+n5<stream_open_is->height))
  {
    n6=x;
    n7=y;
  }
  else if ((x-n4>0)&&(y+n5<stream_open_is->height))
  {
    n6=x-n4;
    n7=y;
  }
  else if ((x-n4>0)&&(y-n5>0))
  {
    n6=x-n4;
    n7=y-n5;
  }
  else if ((x+n4<stream_open_is->width)&&(y-n5>0))
  {
    n6=x;
    n7=y-n5;
  }
  ...
  g_menu_paint_x[g_menu_paint_ptr]=n6;
  g_menu_paint_y[g_menu_paint_ptr]=n7;
  g_menu_paint_n1[g_menu_paint_ptr]=n1;
  g_menu_paint_n2[g_menu_paint_ptr]=n2;
  g_menu_paint_w[g_menu_paint_ptr]=n5;
  g_menu_paint_h[g_menu_paint_ptr]=n4;
  g_menu_paint_menu[g_menu_paint_ptr]=menuptr;
  g_menu_paint_code[g_menu_paint_ptr]=1;
...
int g_paint_menu(void)
{
  ...
  for (i=0;i<g_menu_paint_ptr;i++)
  {
    if (g_menu_paint_code[i]!=1) continue;
    ...
    for (n=0;n<l;n++)
    {
      for (o=0;o<m;o++)
      {
        if ((n<cut)&&(o<cut))   //cut corner
        {
          if (g_cut_map2[cut][n][o]==' ') continue;
          if (g_cut_map2[cut][n][o]=='*')
          {
            deb_set_dot(j+n,k+o,  0,  0,  0);
            continue;
          }
          if (g_cut_map2[cut][n][o]=='.')
          {
            deb_set_dot(j+n,k+o,224,224,224);
            continue;
          }
        }
        ...
        if ((l-n-1<cut)&&(o<cut))
        {
          if (g_cut_map2[cut][l-n-1][o]==' ') continue;
          if (g_cut_map2[cut][l-n-1][o]=='*')
          {
            deb_set_dot(j+n,k+o,  0,  0,  0);
            continue;
          }
        ...
        if ((l-n-1<cut)&&(m-o-1<cut))
        {
          if (g_cut_map2[cut][l-n-1][m-o-1]==' ') continue;
          if (g_cut_map2[cut][l-n-1][m-o-1]=='*')
          {
            deb_set_dot(j+n,k+o,  0,  0,  0);
            continue;
          }
        ...
        if ((n<cut)&&(m-o-1<cut))
        {
          if (g_cut_map2[cut][n][m-o-1]==' ') continue;
          if (g_cut_map2[cut][n][m-o-1]=='*')
          {
            deb_set_dot(j+n,k+o,  0,  0,  0);
            continue;
          }
	...
	t=g_menu_paint_menu[i];
	u=0;

	for (s=g_menu_paint_n1[i];s<=g_menu_paint_n2[i];s++)
	{
	  if ( (g_detect_ptr1==7)&&(g_detect_ptr2==i)&&(g_detect_ptr3==s-g_menu_paint_n1[i])  ||
	       (g_detect_motion_ptr1==7)&&(g_detect_motion_ptr2==i)&&(g_detect_motion_ptr3==s-g_menu_paint_n1[i]) )
	  {
	    if (g_menu_item_separate[t][s]!=1)
	        if ((o-8>=(s-g_menu_paint_n1[i])*16)&&(o-8<(s-g_menu_paint_n1[i]+1)*16))
	          u=1;
	  }
	}
	
	if (u==1) deb_set_dot(j+n,k+o,255-i1,255-i2,255-i3);
	else deb_set_dot(j+n,k+o,i1,i2,i3);
    ...
    if (g_menu_item_separate[t][s]==1)
    {
      x1=j;
      x2=j+l;
      y=k+8+(s-g_menu_paint_n1[i])*16+8;
      
      g_draw_line(x1,y,x2,y,0,192,192,192);
    ...
    x=j+42;
    y=k+1+(s-g_menu_paint_n1[i])*16+8;

    
    strcpy(str,g_menu_item_name[t][s]);
    len=strlen(str);
    j5=0;
...
*/
	      ...
	      if ( (g_detect_ptr1==7)&&(g_detect_ptr2==i)&&(g_detect_ptr3==s-g_menu_paint_n1[i])  ||
	           (g_detect_motion_ptr1==7)&&(g_detect_motion_ptr2==i)&&(g_detect_motion_ptr3==s-g_menu_paint_n1[i]) )
	      {
	        deb_set_dot(x+m2,y+n,255-j1,255-j2,255-j3);
	      }
              else
                deb_set_dot(x+m2,y+n,j1,j2,j3);
/*
 ...
 int ftree_read_dir_ini(void)
{
  ...
  for (i=0;i<MAX_FILE_NUM;i++) ftree_filenamebuff[i][0]=0;

  j=(long long int)getcwd(ftree_currentpath,FN_SIZE);
  if (j==0) return(1);

  ftree_filenamebuff_n=0;
  ftree_filenameplay=0;
  ftree_filenamebuff_ptr=0;

  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ftree_filenamebuff_len2[ftree_filenamebuff_ptr]=30;
  ftree_filenamebuff_ptr++;
  if (ftree_filenamebuff_ptr>=MAX_FILE_NUM) return(0);

  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"C:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ftree_filenamebuff_len2[ftree_filenamebuff_ptr]=30;
  ftree_filenamebuff_ptr++;
  if (ftree_filenamebuff_ptr>=MAX_FILE_NUM) return(0);

  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"D:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"E:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"F:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"G:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"H:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"I:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"J:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"K:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
  ...
  strcpy(ftree_filenamebuff[ftree_filenamebuff_ptr],"L:/");
  //ftree_filenamebuffptr[ftree_filenamebuff_ptr]=2;
  ftree_filenamebuff_type[ftree_filenamebuff_ptr]=0;
  ftree_filenamebuff_icon[ftree_filenamebuff_ptr]=1;
  ftree_filenamebuff_len[ftree_filenamebuff_ptr]=30;
 ...
 int ftree_dir_opened(int ptr )
{
  ...
  if (ptr>=ftree_filenamebuff_ptr) return(0);

  while ((ftree_filenamebuff_subline[ptr+1]==1)&&(ptr+1<ftree_filenamebuff_ptr))
  {
    ptr++;
  }

  i=ftree_get_space(ptr);

  if (ptr+1>=ftree_filenamebuff_ptr) return(0);
  else
  {
    j=ftree_get_space(ptr+1);
    if (j>i) return(1);
    else return(0);
  }
}
 ...
 int ftree_get_space(int ptr)
{
  ...
  return(ftree_filenamebuff_space[ptr]);
}
 ...
 int ftree_get_space2(int ptr)
{
  if (ptr<0) return(0);
  if (ptr>=MAX_FILE_NUM) return(0);
  if (ptr>=ftree_filenamebuff2_ptr) return(0);

  return(ftree_filenamebuff2_space[ptr]);
}
 ...
 int ftree_dir_add_after(int ptr)
{
  ...
  while ((ftree_filenamebuff_subline[ptr]==1)&&(ptr<ftree_filenamebuff_ptr))
  {
    ptr++;
  }
  ...
  ptr--;
    ...
    w = (g_dirview_posi[0][2]-3-4-2) /*cur_stream->width*/  /deb_ch_w;
    leftspace=ftree_get_space(ptr);
	...
	if (deb_str_has_null(ar_buff4[0],FN_SIZE)!=1) continue;
      	...
      	strcpy(ftree_filenamebuff2[ftree_filenamebuff2_ptr],ar_buff4[0]);
        
      	strcpy(ftree_filenamebuff2_ext[ ftree_filenamebuff2_ptr],bt_out_buff3[i]);
      	strcpy(ftree_filenamebuff2_size[ftree_filenamebuff2_ptr],bt_out_buff4[i]);
      	strcpy(ftree_filenamebuff2_date[ftree_filenamebuff2_ptr],bt_out_buff5[i]);

	ftree_filenamebuff2_len[    ftree_filenamebuff2_ptr]=filelen;
	ftree_filenamebuff2_len2[   ftree_filenamebuff2_ptr]=deb_m_info_len;
	ftree_filenamebuff2_type[   ftree_filenamebuff2_ptr]=bt_out_buff2[i];
	ftree_filenamebuff2_icon[   ftree_filenamebuff2_ptr]=bt_out_buff6[i];
	ftree_filenamebuff2_space[  ftree_filenamebuff2_ptr]=leftspace+2;
	ftree_filenamebuff2_subline[ftree_filenamebuff2_ptr]=0;
	
	ftree_filenamebuff2_ptr++;
	if (ftree_filenamebuff2_ptr>=MAX_FILE_NUM) ftree_filenamebuff2_ptr=MAX_FILE_NUM-1;
	
	for (k=1;k<ar_buff4_ptr;k++)
	{
	    if (deb_str_has_null(ar_buff4[k],FN_SIZE)!=1) continue;
	
      	    strcpy(ftree_filenamebuff2[ftree_filenamebuff2_ptr],ar_buff4[k]);
        
      	    ftree_filenamebuff2_ext[ ftree_filenamebuff2_ptr][0]=0;
      	    ftree_filenamebuff2_size[ftree_filenamebuff2_ptr][0]=0;
      	    ftree_filenamebuff2_date[ftree_filenamebuff2_ptr][0]=0;

	    ftree_filenamebuff2_len[    ftree_filenamebuff2_ptr]=filelen;
	    ftree_filenamebuff2_len2[   ftree_filenamebuff2_ptr]=deb_m_info_len;
	    ftree_filenamebuff2_type[   ftree_filenamebuff2_ptr]=bt_out_buff2[i];
	    ftree_filenamebuff2_icon[   ftree_filenamebuff2_ptr]=0;
	    ftree_filenamebuff2_space[  ftree_filenamebuff2_ptr]=leftspace+2;
	    ftree_filenamebuff2_subline[ftree_filenamebuff2_ptr]=1;
  ...
  if (ftree_filenamebuff2_ptr>0)
  {
    l=2;

    for (i=ftree_filenamebuff_ptr-1;i>ptr;i--)
    {
        if (i<0) continue;
        if (i>=MAX_FILE_NUM) continue;

        if (i+ftree_filenamebuff2_ptr<0) continue;
        if (i+ftree_filenamebuff2_ptr>=MAX_FILE_NUM) continue;

        if (deb_str_has_null(ftree_filenamebuff[i],FN_SIZE)!=1) continue;

        //if (strlen(ftree_filenamebuff[i])>=FN_SIZE) continue;

      	strcpy(ftree_filenamebuff[i+ftree_filenamebuff2_ptr],ftree_filenamebuff[i]);

      	strcpy(ftree_filenamebuff_ext[ i+ftree_filenamebuff2_ptr],ftree_filenamebuff_ext[ i]);
      	strcpy(ftree_filenamebuff_size[i+ftree_filenamebuff2_ptr],ftree_filenamebuff_size[i]);
      	strcpy(ftree_filenamebuff_date[i+ftree_filenamebuff2_ptr],ftree_filenamebuff_date[i]);

	ftree_filenamebuff_len[ i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_len[ i];
	ftree_filenamebuff_len2[i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_len2[i];
	ftree_filenamebuff_type[i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_type[i];
	ftree_filenamebuff_icon[i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_icon[i];
	ftree_filenamebuff_space[i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_space[i];
	ftree_filenamebuff_subline[i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_subline[i];
	ftree_filenamebuff_select[i+ftree_filenamebuff2_ptr]=ftree_filenamebuff_select[i];

      	ftree_filenamebuff[i][0]=0;

	ftree_filenamebuff_ext[i][0]=0;
	ftree_filenamebuff_size[i][0]=0;
	ftree_filenamebuff_date[i][0]=0;

	ftree_filenamebuff_len[i]=0;
	ftree_filenamebuff_len2[i]=0;
	ftree_filenamebuff_type[i]=0;
	ftree_filenamebuff_icon[i]=0;
	ftree_filenamebuff_space[i]=0;
	ftree_filenamebuff_subline[i]=0;
	ftree_filenamebuff_select[i]=0;
    }

    for (i=0;i<ftree_filenamebuff2_ptr;i++)
    {
        if (i<0) continue;
        if (i>=MAX_FILE_NUM) continue;
        ...
        if (deb_str_has_null(ftree_filenamebuff2[i],FN_SIZE)!=1) continue;
        ...
        //if (strlen(bt_out_buff[i])>=FN_SIZE) continue;
      	...
      	strcpy(ftree_filenamebuff[ptr+1+i],ftree_filenamebuff2[i]);
        
      	strcpy(ftree_filenamebuff_ext[ ptr+1+i],ftree_filenamebuff2_ext[i]);
      	strcpy(ftree_filenamebuff_size[ptr+1+i],ftree_filenamebuff2_size[i]);
      	strcpy(ftree_filenamebuff_date[ptr+1+i],ftree_filenamebuff2_date[i]);

	ftree_filenamebuff_len[ ptr+1+i]=ftree_filenamebuff2_len[i];
	ftree_filenamebuff_len2[ptr+1+i]=ftree_filenamebuff2_len2[i];
	ftree_filenamebuff_type[ptr+1+i]=ftree_filenamebuff2_type[i];
	ftree_filenamebuff_icon[ptr+1+i]=ftree_filenamebuff2_icon[i];
	ftree_filenamebuff_space[ptr+1+i]=ftree_filenamebuff2_space[i];
	ftree_filenamebuff_subline[ptr+1+i]=ftree_filenamebuff2_subline[i];
	ftree_filenamebuff_select[ptr+1+i]=0;
    }
    
    ftree_filenamebuff_ptr=ftree_filenamebuff_ptr+ftree_filenamebuff2_ptr;
    if (ftree_filenamebuff_ptr>=MAX_FILE_NUM) ftree_filenamebuff_ptr=MAX_FILE_NUM-1;

    if (ptr>=ftree_filenameplay)
    {
    }
    else
    {
      ftree_filenameplay=ftree_filenameplay+ftree_filenamebuff2_ptr;
    }
  }
  else
  {
    ...
    for (i=ftree_filenamebuff_ptr-1;i>ptr;i--)
    {
        ...
        //if (strlen(ftree_filenamebuff[i])>=FN_SIZE) continue;
      	...
      	strcpy(ftree_filenamebuff[i+1],ftree_filenamebuff[i]);

      	strcpy(ftree_filenamebuff_ext[ i+1],ftree_filenamebuff_ext[ i]);
      	strcpy(ftree_filenamebuff_size[i+1],ftree_filenamebuff_size[i]);
      	strcpy(ftree_filenamebuff_date[i+1],ftree_filenamebuff_date[i]);

	ftree_filenamebuff_len[ i+1]=ftree_filenamebuff_len[ i];
	ftree_filenamebuff_len2[i+1]=ftree_filenamebuff_len2[i];
	ftree_filenamebuff_type[i+1]=ftree_filenamebuff_type[i];
	ftree_filenamebuff_icon[i+1]=ftree_filenamebuff_icon[i];
	ftree_filenamebuff_space[i+1]=ftree_filenamebuff_space[i];
	ftree_filenamebuff_subline[i+1]=ftree_filenamebuff_subline[i];
	ftree_filenamebuff_select[i+1]=ftree_filenamebuff_select[i];
    ...
    strcpy(ftree_filenamebuff[ptr+1],"Empty Fold");

    strcpy(ftree_filenamebuff_ext[ptr+1],"    ");
    strcpy(ftree_filenamebuff_size[ptr+1],"      ");
    strcpy(ftree_filenamebuff_date[ptr+1],"                    ");
    
    ftree_filenamebuff_len[ptr+1] =12;
    ftree_filenamebuff_len2[ptr+1] =12;
    ftree_filenamebuff_type[ptr+1]=2;
    ftree_filenamebuff_icon[ptr+1]=0;
    ftree_filenamebuff_space[ptr+1]=leftspace+l;
    ftree_filenamebuff_subline[ptr+1]=0;
    ftree_filenamebuff_select[ptr+1]=0;

    ftree_filenamebuff_ptr=ftree_filenamebuff_ptr+1;
    if (ftree_filenamebuff_ptr>=MAX_FILE_NUM) ftree_filenamebuff_ptr=MAX_FILE_NUM-1;
    ...
    {
      ftree_filenameplay=ftree_filenameplay+1;
    }
  }
 ...
 int ftree_dir_remove_after(int ptr)
{
  ...
  for (j=ptr+1;j<ftree_filenamebuff_ptr;j++)
  {
    if (j<0) continue;
  ...
  if (k==0)
  {
    for (j=ptr+1;j<ftree_filenamebuff_ptr;j++)
    {
      ...
      ftree_filenamebuff[j][0]=0;

      ftree_filenamebuff_ext[j][0]=0;
      ftree_filenamebuff_size[j][0]=0;
      ftree_filenamebuff_date[j][0]=0;

      ftree_filenamebuff_len[j]=0;
      ftree_filenamebuff_len2[j]=0;
      ftree_filenamebuff_type[j]=0;
      ftree_filenamebuff_icon[j]=0;
      ftree_filenamebuff_space[j]=0;
      ftree_filenamebuff_subline[j]=0;
      ftree_filenamebuff_select[j]=0;
    }

    ftree_filenamebuff_ptr=ptr+1;

    ftree_filenameplay=ptr;
  }
  else
  {
    for (j=p1;j<ftree_filenamebuff_ptr;j++)
    {
      if (j<0) continue;
      if (j>=MAX_FILE_NUM) continue;
      ...
      if (deb_str_has_null(ftree_filenamebuff[j],FN_SIZE)!=1) continue;

      //if (strlen(ftree_filenamebuff[j])>=FN_SIZE) continue;

      strcpy(ftree_filenamebuff[ptr+1+j-p1],ftree_filenamebuff[j]);

      strcpy(ftree_filenamebuff_ext[ptr+1+j-p1],ftree_filenamebuff_ext[j]);
      strcpy(ftree_filenamebuff_size[ptr+1+j-p1],ftree_filenamebuff_size[j]);
      strcpy(ftree_filenamebuff_date[ptr+1+j-p1],ftree_filenamebuff_date[j]);

      ftree_filenamebuff_len[ptr+1+j-p1] =ftree_filenamebuff_len[j];
      ftree_filenamebuff_len2[ptr+1+j-p1] =ftree_filenamebuff_len2[j];
      ftree_filenamebuff_type[ptr+1+j-p1]=ftree_filenamebuff_type[j];
      ftree_filenamebuff_icon[ptr+1+j-p1]=ftree_filenamebuff_icon[j];
      ftree_filenamebuff_space[ptr+1+j-p1]=ftree_filenamebuff_space[j];
      ftree_filenamebuff_subline[ptr+1+j-p1]=ftree_filenamebuff_subline[j];
      ftree_filenamebuff_select[ptr+1+j-p1]=ftree_filenamebuff_select[j];
    ...
    for (j=ptr+ftree_filenamebuff_ptr-p1+1;j<p1;j++)
    {
      if (j<0) continue;
      if (j>=MAX_FILE_NUM) continue;
    ...
    ftree_filenamebuff_ptr=ftree_filenamebuff_ptr-(p1-ptr-1);

    if ((ptr>=ftree_filenameplay)&&(p1>=ftree_filenameplay))
    {
    }
    else
    {
      if ((ptr<ftree_filenameplay)&&(p1<=ftree_filenameplay))
      {
        ftree_filenameplay=ftree_filenameplay-(p1-ptr-1);
      }
      else
      {
        ftree_filenameplay=ptr;
 ...
 int ftree_get_path(int ptr)
{
  ...
  ftree_dir_buffer[0]=0;

  ns1=ftree_get_space(ptr);

  while (p1>=0)
  {
    if (ns1<2)
    {
      strcpy(m5_buffer1,ftree_filenamebuff[p1]);
        
      i=p1+1;
      while ((ftree_filenamebuff_subline[i]==1)&&(i<ftree_filenamebuff_ptr))
      {
	if (strlen(m5_buffer1)+strlen(ftree_filenamebuff[i])>=FN_SIZE) break;
		    
        strcat(m5_buffer1,ftree_filenamebuff[i]);
        i++;
      }
        
      ftree_get_path2(m5_buffer1,m5_buffer2);
    ...
    else
    {
	  ...
	  if (strlen(m5_buffer1)+strlen(ftree_filenamebuff[i])>=FN_SIZE) break;
        ...
        ftree_get_path1(m5_buffer1,m5_buffer2);
        strcpy(m5_buffer3,m5_buffer2);
      }
    }
    ...
    while(1)
    {
      ...
      else
      {
        ...
        while ((ftree_filenamebuff_subline[p1]==1)&&(p1>0))
        {
          p1--;
        }
  ...
  strcpy(ftree_dir_buffer,m5_buffer4);
...
*/
 ...
 int ftree_filename_ext(char *name,int name_size,char *fext,int fext_size)
{
  int  i,j,k,l;
...
/*
static int ftree_filenameext2(char *path,int path_size,char *fext,int fext_size)
{
  int  i,j,k,l;
  struct stat info;
...
static int    out_buffer_tmp_len;
 ...
 int deb_utf8_to_gb18030(char *in_buffer,char *out_buffer,int out_buffer_len)
{
    int i;
    //iconv_t cd;
    ...
    string_cpy(out_buffer,out_buffer_len,m501_str2,5000);    

    return(0);  
}

 int deb_gb18030_to_utf8(char *in_buffer,char *out_buffer,int out_buffer_len)
{
    int i,j;
...
static char disp_buff2[350][300];
static int  disp_leftspace[350];
static int  disp_icon[350];
static int  disp_selected[350];
static int  disp_len[350];
static char m11_str1[6000];
//static char m11_str2[6000];
//static char m11_str3[6000];
//static char m11_str4[6000];

 int ftree_disp_dir(VideoState *cur_stream)
{
  int  i,i1,j,k,l,m;
  int  h,w;
  int  leftspace,dirlen,filelen,selected,first,focus;
  int  n1,n4,n5,n6,n7,n8,n9,n10;
  char *strptr;
  int  icon;
  int  x,y;
  ...
  h = g_dirview_posi[0][3] /*cur_stream->height*/ /(deb_ch_h+2);
  w = (g_dirview_posi[0][2]-3-4-2) /*cur_stream->width*/  /deb_ch_w;
  ...
  for (n4=0;n4<h;n4++)
  {
    if (ftree_filenamebuff_n+n4<0) continue;
    if (ftree_filenamebuff_n+n4>=MAX_FILE_NUM) continue;
    if (ftree_filenamebuff_n+n4>=ftree_filenamebuff_ptr) continue;

    if (deb_str_has_null(ftree_filenamebuff[ftree_filenamebuff_n+n4],FN_SIZE)!=1) continue;

    strcpy(m11_str1,ftree_filenamebuff[ftree_filenamebuff_n+n4]);

    if (deb_str_has_null(m11_str1,6000)!=1) continue;

    m=ftree_filenamebuff_n+n4;
    while((ftree_filenamebuff_subline[m]==1)&&(m>0)) m--;
    
    selected=ftree_filenamebuff_select[m];
    
    icon=ftree_filenamebuff_icon[ftree_filenamebuff_n+n4];

    leftspace=ftree_get_space(ftree_filenamebuff_n+n4);
    dirlen =ftree_filenamebuff_len[ftree_filenamebuff_n+n4];
    ...
    if (filelen<1) continue;
    
    disp_leftspace[n4]=leftspace;
    disp_icon[n4]=icon;
    disp_selected[n4]=selected;
    disp_len[n4]=filelen;
    ...
    if (ftree_filenamebuff_subline[ftree_filenamebuff_n+n4]==0)
    {
      strcpy(disp_buff2[n4],"  ");
      strcat(disp_buff2[n4],ftree_filenamebuff_ext[ ftree_filenamebuff_n+n4]);
      strcat(disp_buff2[n4],"  ");
      strcat(disp_buff2[n4],ftree_filenamebuff_size[ftree_filenamebuff_n+n4]);
      strcat(disp_buff2[n4],"  ");
      strcat(disp_buff2[n4],ftree_filenamebuff_date[ftree_filenamebuff_n+n4]);
    }
  }
  ...
  //deb_fill_rectangle( 0, 0, cur_stream->width, deb_ch_h*h); // daipozhi modified 
  ...
  for (n4=0;n4<h;n4++)
  {
    ...
    if (ftree_filenamebuff_subline[ftree_filenamebuff_n+n4]==0)
    {
      g_paint_icon(x+3+n5*7,y+n4*(deb_ch_h+2)+1 ,n6);
    }
    
    if (disp_selected[n4]==0) u_echo_str4screenstring(x+3+n5*7+2*deb_ch_w+4+1 ,y+n4*(deb_ch_h+2)+1 ,strptr ,6000,0,0);
    else                      u_echo_str4screenstring(x+3+n5*7+2*deb_ch_w+4+1 ,y+n4*(deb_ch_h+2)+1 ,strptr ,6000,3,0);

    if (ftree_filenamebuff_subline[ftree_filenamebuff_n+n4]==0)
    {
        u_echo_str4screenstring(x+3+n5*7+2*deb_ch_w+4+1+disp_len[n4]*7 ,y+n4*(deb_ch_h+2)+1 ,disp_buff2[n4] ,300,0,0);
    }
  }

  i=ftree_filenameplay+1;
  j=0;
  ...
  for (k=ftree_filenameplay;k<=ftree_filenameplay+j;k++)
  {
    n4=k-ftree_filenamebuff_n;
    if ((n4>=0)&&(n4<h)&&(n4<350)) 
    {
      if (first==1)
      {
  ...
  if (focus==1)
  {
        ...
        //if (disp_selected[n4]==0) u_echo_str4screenstring(x+3+n5*7+2*deb_ch_w+4+1 ,y+n4*(deb_ch_h+2)+1 ,strptr ,6000,1,0);
        //else                      u_echo_str4screenstring(x+3+n5*7+2*deb_ch_w+4+1 ,y+n4*(deb_ch_h+2)+1 ,strptr ,6000,4,0);
        
        n10=0;   //draw focus
        
        for (i=x+3+n5*7+2*deb_ch_w+4+1-1;i<x+3+n5*7+2*deb_ch_w+4+1+n9*7+1;i++)
        {
            if ((n10/2)*2==n10) i1=0;
            else                i1=255;
            
            deb_set_dot(i,y+n7*(deb_ch_h+2)+1-1,i1,i1,i1);
            
            n10++;
        }

        for (j=y+n7*(deb_ch_h+2)+1-1;j<y+(n8+1)*(deb_ch_h+2)-1;j++)
        {
            if ((n10/2)*2==n10) i1=0;
            else                i1=255;
            
            deb_set_dot(x+3+n5*7+2*deb_ch_w+4+1+n9*7+1,j,i1,i1,i1);
            
            n10++;
        }

        for (i=x+3+n5*7+2*deb_ch_w+4+1+n9*7+1;i>x+3+n5*7+2*deb_ch_w+4+1-1;i--)
        {
            if ((n10/2)*2==n10) i1=0;
            else                i1=255;
            
            deb_set_dot(i,y+(n8+1)*(deb_ch_h+2)-1,i1,i1,i1);
            
            n10++;
        }

        for (j=y+(n8+1)*(deb_ch_h+2)-1;j>y+n7*(deb_ch_h+2)+1-1;j--)
        {
            if ((n10/2)*2==n10) i1=0;
            else                i1=255;
            
            deb_set_dot(x+3+n5*7+2*deb_ch_w+4+1-1,j,i1,i1,i1);
 ...
 int ftree_conv_dir(void)
{
    ...
    while (i<ftree_filenamebuff_ptr)
    {
        k=i;
        strcpy(str1,ftree_filenamebuff[i]);

        leftspace=ftree_get_space(i);
	dirlen =ftree_filenamebuff_len2[i];
        ...
        while ((ftree_filenamebuff_subline[i]==1)&&(i<ftree_filenamebuff_ptr))
        {
            if (strlen(str1)+strlen(ftree_filenamebuff[i])>=FN_SIZE) break;
            
            strcat(str1,ftree_filenamebuff[i]);
            i++;
        }
	...
	if (deb_str_has_null(ar_buff4[0],FN_SIZE)!=1) continue;
      	...
      	strcpy(ftree_filenamebuff2_ext[ ftree_filenamebuff2_ptr],ftree_filenamebuff_ext[k]);
      	strcpy(ftree_filenamebuff2_size[ftree_filenamebuff2_ptr],ftree_filenamebuff_size[k]);
      	strcpy(ftree_filenamebuff2_date[ftree_filenamebuff2_ptr],ftree_filenamebuff_date[k]);

	ftree_filenamebuff2_len[    ftree_filenamebuff2_ptr]=filelen;
	ftree_filenamebuff2_len2[   ftree_filenamebuff2_ptr]=ftree_filenamebuff_len2[k];
	ftree_filenamebuff2_type[   ftree_filenamebuff2_ptr]=ftree_filenamebuff_type[k];
	ftree_filenamebuff2_icon[   ftree_filenamebuff2_ptr]=ftree_filenamebuff_icon[k];
	ftree_filenamebuff2_space[  ftree_filenamebuff2_ptr]=ftree_filenamebuff_space[k];
	ftree_filenamebuff2_subline[ftree_filenamebuff2_ptr]=0;
	ftree_filenamebuff2_select[ftree_filenamebuff2_ptr]=ftree_filenamebuff_select[k];
	...
	for (l=1;l<ar_buff4_ptr;l++)
	{
	    if (deb_str_has_null(ar_buff4[l],FN_SIZE)!=1) continue;
      	    ...
      	    strcpy(ftree_filenamebuff2[ftree_filenamebuff2_ptr],ar_buff4[l]);
    ...
    // find new window top
    i=ftree_filenamebuff_n;
    
    while ((ftree_filenamebuff_subline[i]==1)&&(i>0))
    {
      i--;
    }
    
    ftree_filenamebuff_n2=ftree_filenamebuff_n-i;
    
    j=0;
    
    for (i=0;i<=ftree_filenamebuff_n;i++)
    {
        if (ftree_filenamebuff_subline[i]==0) j++;
    }
    ...
    for (i=0;i<ftree_filenamebuff2_ptr;i++)
    {
        if (ftree_filenamebuff2_subline[i]==0) k++;
        if (k>=j) break;
    }
    ...
    while ((ftree_filenamebuff2_subline[m]==1)&&(m<ftree_filenamebuff2_ptr))
    {
        n++;
        m++;
    }
    
    if (n>ftree_filenamebuff_n2) o=ftree_filenamebuff_n2;
    else o=n;
    
    ftree_filenamebuff_n =l+o;
    ftree_filenamebuff_n2=o;

    // find new play id
    i=ftree_filenameplay;
    ...
    for (i=0;i<=ftree_filenameplay;i++)
    {
        if (ftree_filenamebuff_subline[i]==0) j++;
    }
    ...
    for (i=0;i<ftree_filenamebuff2_ptr;i++)
    {
        if (deb_str_has_null(ftree_filenamebuff2[i],FN_SIZE)!=1) continue;
      	...
      	strcpy(ftree_filenamebuff[i],ftree_filenamebuff2[i]);

      	strcpy(ftree_filenamebuff_ext[ i],ftree_filenamebuff2_ext[ i]);
      	strcpy(ftree_filenamebuff_size[i],ftree_filenamebuff2_size[i]);
      	strcpy(ftree_filenamebuff_date[i],ftree_filenamebuff2_date[i]);

	ftree_filenamebuff_len[    i]=ftree_filenamebuff2_len[    i];
	ftree_filenamebuff_len2[   i]=ftree_filenamebuff2_len2[   i];
	ftree_filenamebuff_type[   i]=ftree_filenamebuff2_type[   i];
	ftree_filenamebuff_icon[   i]=ftree_filenamebuff2_icon[   i];
	ftree_filenamebuff_space[  i]=ftree_filenamebuff2_space[  i];
	ftree_filenamebuff_subline[i]=ftree_filenamebuff2_subline[i];
	ftree_filenamebuff_select[i]=ftree_filenamebuff2_select[i];
    }

    p=ftree_filenamebuff_ptr;
    ftree_filenamebuff_ptr=ftree_filenamebuff2_ptr;
    
    //clear memory
    for (j=ftree_filenamebuff_ptr;j<p;j++)
    {
      if (j<0) continue;
      if (j>=MAX_FILE_NUM) continue;
...
/*
static int ftree_read_dir_len(int ptr)
{
    int i,j,k;
    ...
    while ((ftree_filenamebuff_subline[ptr]==1)&&(ptr>0))
    {
      ptr--;
    }
...
*/
/*
static int ftree_read_dir_len2(int ptr)
{
    int i,j,k;
    ...
    while ((ftree_filenamebuff2_subline[ptr]==1)&&(ptr>0))
    {
      ptr--;
    }
 ...
 int  ftree_read_dir(void)
{
  ...
  k=(long long int)getcwd(ftree_currentpath,FN_SIZE);
  if (k==0) return(1);

  if (dirp = opendir(ftree_currentpath))
  {
    while (1)
    {
	...
	i=ftree_filename_dir(ftree_currentpath,entry->d_name);
	...
	else
	{
	  ...
	  // ext
	  ftree_filename_ext(entry->d_name,256,m202_buffer1,3000);
	  ...
	  else if (deb_m_info.st_size>=1000000000000)
	  {
	    ...
	    ftree_size_format(j,m202_buffer4);
...
/*  
  printf("fnc inp str:%s,\n",p_str);
  printf("fnc out str:%s,\n",fnc_str);
  printf("fnc out msk:%s,\n",fnc_msk);
...
int ar_conv(char *p_in_str,int p_in_str_size,int p_len)
{
  ...
  string_cpy(ar_buff3,1000,p_in_str,p_in_str_size);    
          ...
          for (n=0;n<u_char_size_y;n++)
          {
            ...
            if (st==0)  // normal
            {
                if (vid)
                {
                  ...
                  deb_fill_rectangle(x+m, y+n, 1, 1);
                }
                else
                {
            ...
            else if (st==1) //black(selected and playing)
            {
            ...
            else if (st==2) // green(for sound river)
            {
               deb_set_dot(x+m,y+n,0,255-i1,0);
            }
            else if (st==3) // blue(selected)
            {
               i0=255-i1;
            ...
            else if (st==4) // deep blue(playing)
            {
               i0=255-i1;
...
int u_test_disp_win(void)
{
    ...
    for (i=0;i<32;i++)
    {
      u_echo_str4screenstring(0,14*i,u_test_buf[i],300,0,1);
    }
